...
> Let me first confirm the definition so I don't misapply it: in a polygon, a diagonal is a line segment connecting two non-adjacent vertices, which means we exclude the sides of the polygon itself from the count [^notfound].

> Next, I should review the general formula. The number of diagonals in an n-sided polygon is given by n(n − 3)/2, because each vertex connects to n − 3 other vertices via diagonals (excluding itself and its two adjacent vertices), and we divide by 2 to avoid double-counting each diagonal [^notfound].

> Now, I will substitute n = 6 into the formula: 6 × (6 − 3) / 2 = 6 × 3 / 2 = 18 / 2 = 9. Wait, let me verify the arithmetic: 6 times 3 is 18, divided by 2 is indeed 9, so the calculation is correct [^notfound].
...
> I will now examine the answer choices: A. 6, B. 8, C. 9, D. 36. My computed value of 9 matches option C, whereas 6 would be the number of sides, 8 is close but incorrect, and 36 is far too large and would correspond to something like 6 squared rather than the diagonal formula, so the correct choice is C [^notfound].

---

The number of distinct diagonals in a convex hexagon is **9**. This is calculated using the formula for diagonals in an n-sided polygon: (\\frac{n(n-3)}{2}), which for n = 6 gives (\\frac{6 \\times 3}{2} = 9). The correct answer is **C. 9**.
