...
Form the equation of the line passing through the point $A(0 ; 7)$ and tangent to the circle $(x-15)^{2}+(y-2)^{2}=25$.
...
The equations of the lines through (A(0, 7)) tangent to the circle ((x-15)^2 + (y-2)^2 = 25) are:

- **Horizontal tangent**: (y = 7)
- **Sloped tangent**: (y = -\\frac{3}{4}x + 7)
...
[
\\frac{|m \\cdot 15–1 \\cdot 2 + 7|}{\\sqrt{m^2 + (-1)^2}} = 5
]

Simplify the numerator:

[
\\frac{|15m + 5|}{\\sqrt{m^2 + 1}} = 5
]
...
[
m = 0 \\quad \\text{or} \\quad m = -\\frac{3}{4}
]
...
[
AC = \\sqrt{(15–0)^2 + (2–7)^2} = \\sqrt{225 + 25} = \\sqrt{250} = 5\\sqrt{10}
]
...
[
AT = \\sqrt{AC^2 - r^2} = \\sqrt{250–25} = \\sqrt{225} = 15
]
...
[
\\sin \\theta = \\frac{r}{AC} = \\frac{5}{5\\sqrt{10}} = \\frac{1}{\\sqrt{10}}
]

The slope of (AC) is:

[
m_{AC} = \\frac{2–7}{15–0} = -\\frac{1}{3}
]

Using the angle-addition formula for slopes, the tangent slopes are:

[
m = \\frac{m_{AC} \\pm \\tan \\theta}{1 \\mp m_{AC} \\tan \\theta}
]

Compute (\\tan \\theta = \\frac{\\sin \\theta}{\\cos \\theta} = \\frac{1/\\sqrt{10}}{3/\\sqrt{10}} = \\frac{1}{3}), then:

[
m = \\frac{-\\frac{1}{3} \\pm \\frac{1}{3}}{1 \\mp (-\\frac{1}{3})(\\frac{1}{3})} = \\frac{-\\frac{1}{3} \\pm \\frac{1}{3}}{1 \\pm \\frac{1}{9}}
]

This yields (m = 0) and (m = -\\frac{3}{4}), matching the algebraic result.
...
The equations of the tangent lines through (A(0, 7)) to the circle are (y = 7) and (y = -\\frac{3}{4}x + 7).
...
The plot displays the logarithmic derivative of the average spatial extent of the random walk as a function of the inverse logarithm of the number of steps N. Data shown for σ = 1.8 and σ = 1.1, corresponding to ρ = 3.8 (top) and ρ = 3.1 (bottom), for different lattice sizes: L × L = 64 × 64 (blue), 128 × 128 (yellow), 256 × 256 (green). The collapse region is well established for larger values of ρ and corresponds to a regime where scaling corrections and finite sizes effects are negligible, so that one can define a `confidence region', as bracketed by the vertical red dashed lines, and extrapolate from there the critical exponent.
