...
### Rational strain design with minimal phenotype perturbation [^115Y9nnU]. Nature Communications (2024). High credibility.
...
### Globalizing manifold-based reduced models for equations and data [^114MwD53]. Nature Communications (2025). High credibility.
...
### Globalizing manifold-based reduced models for equations and data [^111CGJMR]. Nature Communications (2025). High credibility.
...
### Effective plans for hospital system response to earthquake emergencies [^115d7VYR]. Nature Communications (2020). High credibility.
...
### Learning interpretable network dynamics via universal neural symbolic regression [^116mKmeM]. Nature Communications (2025). High credibility.
...
### Bias free multiobjective active learning for materials design and discovery [^113uYcT1]. Nature Communications (2021). High credibility.
...
### Optimal enzyme utilization suggests that concentrations and thermodynamics determine binding mechanisms and enzyme saturations [^111za8xS]. Nature Communications (2023). High credibility.
...
### Heuristic identification of biological architectures for simulating complex hierarchical genetic interactions [^111czsih]. Genetic Epidemiology (2015). Low credibility.
...
### Active learning framework with iterative clustering for bioimage classification [^114dx9av]. Nature Communications (2012). Medium credibility.
...
### Rational strategies for improving the efficiency of design and discovery of nanomedicines [^115Kim4S]. Nature Communications (2024). High credibility.
...
### Does temporal irregularity drive prediction failure in schizophrenia? temporal modelling of ERPs [^115kyGyk]. Schizophrenia (2022). Medium credibility.
...
### Rational strategies for improving the efficiency of design and discovery of nanomedicines [^114weCKh]. Nature Communications (2024). High credibility.
...
### Rationally inattentive intertemporal choice [^1161RcsG]. Nature Communications (2020). High credibility.
...
### An event-based architecture for solving constraint satisfaction problems [^113Ez9QH]. Nature Communications (2015). Medium credibility.
...
### Use of machine learning to differentiate children with Kawasaki disease from other febrile children in a pediatric emergency department [^112U4qjo]. JAMA Network Open (2023). High credibility.
...
### Resilient cooperators stabilize long-run cooperation in the finitely repeated prisoner's dilemma [^116vUAoL]. Nature Communications (2017). Medium credibility.
...
### Parametric and nonparametric statistical methods for genomic selection of traits with additive and epistatic genetic architectures [^11547GNV]. G3 (2014). Low credibility.
...
### Selecting fitted models under epistemic uncertainty using a stochastic process on quantile functions [^117HexMy]. Nature Communications (2025). High credibility.
...
### Finding spatially variable ligand-receptor interactions with functional support from downstream genes [^1155ecRs]. Nature Communications (2025). High credibility.
...
### One-shot learning for solution operators of partial differential equations [^116VMm91]. Nature Communications (2025). High credibility.
...
### AmpliconReconstructor integrates NGS and optical mapping to resolve the complex structures of focal amplifications [^113257Ex]. Nature Communications (2020). High credibility.
...
### CurveCurator: a recalibrated F-statistic to assess, classify, and explore significance of dose-response curves [^115cmvEC]. Nature Communications (2023). High credibility.
...
### Optimal free-surface pumping by an undulating carpet [^116XmHjJ]. Nature Communications (2023). High credibility.
...
### Learnable latent embeddings for joint behavioural and neural analysis [^111sj1Eo]. Nature (2023). Excellent credibility.
...
### Optimal diversification strategies in the networks of related products and of related research areas [^113UGrHV]. Nature Communications (2018). Medium credibility.
...
### Nonlinear delay differential equations and their application to modeling biological network motifs [^1122i69J]. Nature Communications (2021). High credibility.
...
### Automated in vivo enzyme engineering accelerates biocatalyst optimization [^115oZQPb]. Nature Communications (2024). High credibility.
...
### One-shot learning for solution operators of partial differential equations [^1145wJxy]. Nature Communications (2025). High credibility.
...
### Deep domain adversarial learning for species-agnostic classification of histologic subtypes of osteosarcoma [^116vz8Vt]. The American Journal of Pathology (2023). Medium credibility.
...
### Identifying domains of applicability of machine learning models for materials science [^114x1AgC]. Nature Communications (2020). High credibility.
...
### Variable number tandem repeats mediate the expression of proximal genes [^115hPMJK]. Nature Communications (2021). High credibility.
...
### Design principles for enhancing phase sensitivity and suppressing phase fluctuations simultaneously in biochemical oscillatory systems [^113hBzpx]. Nature Communications (2018). Medium credibility.
...
### Universal structures for adaptation in biochemical reaction networks [^113WAPBy]. Nature Communications (2023). High credibility.
...
### The connectome-based prediction of trust propensity in older adults: a resting-state functional magnetic resonance imaging study [^1128bp9s]. Human Brain Mapping (2023). Medium credibility.
...
### Large-scale mapping and mutagenesis of human transcriptional effector domains [^112kffms]. Nature (2023). Excellent credibility.
...
### A hierarchy of biomolecular proportional-integral-derivative feedback controllers for robust perfect adaptation and dynamic performance [^113vDJiu]. Nature Communications (2022). High credibility.
...
### Higher-order and distributed synergistic functional interactions encode information gain in goal-directed learning [^114ZKsmR]. Nature Communications (2025). High credibility.
...
### Universal structures for adaptation in biochemical reaction networks [^114jTMPv]. Nature Communications (2023). High credibility.
...
### The topological requirements for robust perfect adaptation in networks of any size [^117U3jt9]. Nature Communications (2018). Medium credibility.
...
### A rational multi-target combination strategy for synergistic improvement of non-ribosomal peptide production [^112vBoCr]. Nature Communications (2025). High credibility.
...
### Combinatorial code governing cellular responses to complex stimuli [^116xbLcM]. Nature Communications (2015). Medium credibility.
...
### Selecting fitted models under epistemic uncertainty using a stochastic process on quantile functions [^114AYiYz]. Nature Communications (2025). High credibility.
...
### Universal structures for adaptation in biochemical reaction networks [^1149YsqZ]. Nature Communications (2023). High credibility.
...
### A robustness-based approach to systems-oriented drug design [^112kN5JV]. Nature Reviews: Drug Discovery (2007). Medium credibility.
...
### Universal light-guiding geometry for on-chip resonators having extremely high Q-factor [^111ig4X7]. Nature Communications (2020). High credibility.
...
### High-throughput screening and rational design of biofunctionalized surfaces with optimized biocompatibility and antimicrobial activity [^115DcE6U]. Nature Communications (2021). High credibility.
...
### Mechanics of drosophila wing deployment [^112kgpwP]. Nature Communications (2024). High credibility.
...
### Classification-based inference of dynamical models of gene regulatory networks [^1135LPDm]. G3 (2019). Medium credibility.
...
### Classification-based inference of dynamical models of gene regulatory networks [^116svY5C]. G3 (2019). Medium credibility.
...
### Homoscedasticity: an overlooked critical assumption for linear regression [^115ByXQ1]. General Psychiatry (2019). Medium credibility.
...
### One-shot learning for solution operators of partial differential equations [^115yifbW]. Nature Communications (2025). High credibility.
...
### Non-linear relationships in clinical research [^112jo8vG]. Nephrology, Dialysis, Transplantation (2025). Medium credibility.
...
### Computational frameworks transform antagonism to synergy in optimizing combination therapies [^111bxvpg]. NPJ Digital Medicine (2025). Medium credibility.
...
### Optimal enzyme utilization suggests that concentrations and thermodynamics determine binding mechanisms and enzyme saturations [^111H1TMW]. Nature Communications (2023). High credibility.
