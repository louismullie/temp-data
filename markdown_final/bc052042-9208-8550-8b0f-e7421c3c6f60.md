...
- **Homogeneity over integers**: (f(n x) = n f(x)) for all (n ∈ ℤ) and (x \\in \\mathbb{Q}).
...
Let (r = [m/n] ∈ ℚ) with (m \\in \\mathbb{Z}) and (n \\in \\mathbb{Z} \\setminus {0}). We show (f(r x) = r f(x)):
