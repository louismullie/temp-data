...
2b = a + (3a − b) ⇒ 2b = 4a − b ⇒ 3b = 4a ⇒ b = [4a/3]
...
- (A([1/3], [1/a]))
- (B([1/4], [1/b]) = B([1/4], [3/(4a)]))
- (C([1/5], [1/c]) = C([1/5], [3/(5a)]))
...
slope_AB = [(3/(4a) − 1/a)/(1/4 − 1/3)] = [(−1/(4a))/(−1/12)] = [3/a]

Calculate the slope of BC:

slope_BC = [(3/(5a) − 3/(4a))/(1/5 − 1/4)] = [(−3/(20a))/(−1/20)] = [3/a]

Since slope_AB = slope_BC = [3/a], the points are **collinear**.
...
### Towards expert-level autonomous carotid ultrasonography with large-scale learning-based robotic system [^115o5pX6]. Nature Communications (2025). High credibility.
...
### Mean-shift exploration in shape assembly of robot swarms [^112ynzyQ]. Nature Communications (2023). High credibility.
...
### Accurate global and local 3D alignment of cryo-EM density maps using local spatial structural features [^116UZU51]. Nature Communications (2024). High credibility.
...
### Efficient self-organization of informal public transport networks [^117Mbfcg]. Nature Communications (2024). High credibility.
...
### An introduction to machine learning [^1179nseq]. Clinical Pharmacology and Therapeutics (2020). Medium credibility.
...
### Enhancing endoscopic measurement: validating a quantitative method for polyp size and location estimation in upper gastrointestinal endoscopy [^111gh2PV]. Surgical Endoscopy (2024). Medium credibility.
...
### Hierarchical organic microspheres from diverse molecular building blocks [^114FZWjH]. Nature Communications (2024). High credibility.
...
### Accurate global and local 3D alignment of cryo-EM density maps using local spatial structural features [^1165L7p9]. Nature Communications (2024). High credibility.
...
### Metasurface-driven full-space structured light for three-dimensional imaging [^115BmR9R]. Nature Communications (2022). High credibility.
...
### Evaluating continuous curvilinear capsulorhexis proficiency via video analysis [^113Az2G2]. BMJ Open Ophthalmology (2025). High credibility.
...
### Filopodia-based contact stimulation of cell migration drives tissue morphogenesis [^112m7TEJ]. Nature Communications (2021). High credibility.
...
### Spine localization in X-ray images using interest point detection [^114nZ3MC]. Journal of Digital Imaging (2009). Low credibility.
...
### Machine learning for cluster analysis of localization microscopy data [^115RU7Un]. Nature Communications (2020). High credibility.
...
### Semiautomatic segmentation of vertebrae in lateral X-rays using a conditional shape model [^117Cm916]. Academic Radiology (2007). Low credibility.
...
### Accurate global and local 3D alignment of cryo-EM density maps using local spatial structural features [^1167N23j]. Nature Communications (2024). High credibility.
...
### An improved spinning lens test to determine the stiffness of the human lens [^114JNr8d]. Experimental Eye Research (2011). Low credibility.
...
### Optically guided patient positioning techniques [^115xxTbo]. Seminars in Radiation Oncology (2005). Low credibility.
...
### A novel theory for rapid localization of the transverse-sigmoid sinus junction and "keyhole" in the retrosigmoid keyhole approach: micro-anatomical study, technique nuances, and clinical application [^113HEGWw]. Neurosurgical Review (2024). Medium credibility.
...
### Human navigation strategies and their errors result from dynamic interactions of spatial uncertainties [^114W6BW8]. Nature Communications (2024). High credibility.
...
### Accurate global and local 3D alignment of cryo-EM density maps using local spatial structural features [^111kDBng]. Nature Communications (2024). High credibility.
...
### Accurate global and local 3D alignment of cryo-EM density maps using local spatial structural features [^113z7mWx]. Nature Communications (2024). High credibility.
...
### Relationship between myopia control and amount of corneal refractive change after orthokeratology lens treatment [^115r2A6a]. BMC Ophthalmology (2023). Medium credibility.
...
### ACR-ARS practice parameter for the performance of stereotactic body radiation therapy [^111X2jBM]. ARS/ACR (2024). High credibility.
...
### Heading direction with respect to a reference point modulates place-cell activity [^113RrThJ]. Nature Communications (2019). High credibility.
...
### A comparison of skeletal muscle diffusion tensor imaging tractography seeding methods [^113EMHbU]. NMR in Biomedicine (2025). Medium credibility.
...
### Fiducial point placement and the accuracy of point-based, rigid body registration [^117H3S8C]. Neurosurgery (2001). Low credibility.
...
### Three-dimensional determination of variability in colon anatomy: applications for numerical modeling of the intestine [^111o73TF]. The Journal of Surgical Research (2012). Low credibility.
...
### Three-dimensional printing template for intraoperative localization of pulmonary nodules in the pleural cavity [^115NhjWk]. JTCVS Techniques (2022). Medium credibility.
...
### Different strategies in pointing tasks and their impact on clinical bedside tests of spatial orientation [^117NYiTV]. Journal of Neurology (2022). Medium credibility.
...
### Different strategies in pointing tasks and their impact on clinical bedside tests of spatial orientation [^111cB7Fx]. Journal of Neurology (2022). Medium credibility.
...
### Rating certainty when the target threshold is the null and the point estimate is close to the null [^111Z3Bi1]. BMJ Evidence-Based Medicine (2025). High credibility.
...
### A framework for evaluating correspondence between brain images using anatomical fiducials [^1115mpp7]. Human Brain Mapping (2019). Medium credibility.
...
### Heading direction with respect to a reference point modulates place-cell activity [^115N3ywB]. Nature Communications (2019). High credibility.
...
### Constrained optimization for position calibration of an NMR field camera [^112PKNdk]. Magnetic Resonance in Medicine (2018). Low credibility.
...
### Time course of tumor SUV in 18F-FDG PET of breast cancer: presentation of a simple model using a single reference point for time corrections of tumor SUVs [^115EhtAe]. Journal of Nuclear Medicine (2011). Low credibility.
...
### Three-dimensional cephalometry: a method for the identification and for the orientation of the skull after cone-bean computed tomographic scan [^116GBL5B]. The Journal of Craniofacial Surgery (2013). Low credibility.
...
### Automated sequence design of 2D wireframe DNA origami with honeycomb edges [^113yyfxT]. Nature Communications (2019). High credibility.
...
### Novel cardiac CT method for identifying the atrioventricular conduction axis by anatomic landmarks [^114jwi4C]. Heart Rhythm (2025). Medium credibility.
...
### Heading direction with respect to a reference point modulates place-cell activity [^112HrUhJ]. Nature Communications (2019). High credibility.
...
### Heading direction with respect to a reference point modulates place-cell activity [^1173v8NN]. Nature Communications (2019). High credibility.
...
### Meshless deformable models for 3D cardiac motion and strain analysis from tagged MRI [^115o3Ebk]. Magnetic Resonance Imaging (2015). Low credibility.
...
### ACR-ARS practice parameter for image-guided radiation therapy (IGRT) [^114X9UgF]. ARS/ACR (2024). High credibility.
...
### A consensus document on robotic surgery [^111VrWoX]. Surgical Endoscopy (2008). Medium credibility.
...
### Methods to locate center of gravity in scoliosis [^114Q8B4i]. Spine (2003). Low credibility.
...
### ACR-ARS practice parameter for image-guided radiation therapy (IGRT) [^116h1GAP]. American Journal of Clinical Oncology (2025). High credibility.
...
### Retinal image registration using geometrical features [^112oXeKm]. Journal of Digital Imaging (2013). Low credibility.
...
### Different strategies in pointing tasks and their impact on clinical bedside tests of spatial orientation [^1169Uznx]. Journal of Neurology (2022). Medium credibility.
...
### Ultrasound evaluation of a new surface reference line to describe sural nerve location and safe zones to consider in posterior leg approaches [^117TQ95C]. Knee Surgery, Sports Traumatology, Arthroscopy (2023). Medium credibility.
...
### Stereodivergent 1, 3-difunctionalization of alkenes by charge relocation [^116r4Xoa]. Nature (2024). Excellent credibility.
...
### Descent trajectory reconstruction and landing site positioning of chang' E-4 on the lunar farside [^113RXs9t]. Nature Communications (2019). High credibility.
...
### Males but not females report genital sensations evoked by fixed-parameter stimulation of somatosensory cortex [^114ftwWr]. Brain (2025). Medium credibility.
...
### A bedside application-based assessment of spatial orientation and memory: approaches and lessons learned [^116w92nD]. Journal of Neurology (2019). Medium credibility.
...
### An adaptive off-line procedure for radiotherapy of prostate cancer [^115Yu1Gc]. International Journal of Radiation Oncology, Biology, Physics (2007). Low credibility.
...
### Human navigation strategies and their errors result from dynamic interactions of spatial uncertainties [^113MpEsD]. Nature Communications (2024). High credibility.
...
### ACR-ARS practice parameter for the performance of stereotactic body radiation therapy [^113ZhU7Q]. ARS/ACR (2024). High credibility.
...
### ACR-ARS practice parameter for the performance of brain stereotactic radiosurgery [^113dcpsF]. ARS/ACR (2021). High credibility.
...
### Rimabotulinumtoxinb (Myobloc) [^117KkBX6]. FDA (2025). Medium credibility.
...
### Simultaneous 3D localization of multiple MR-visible markers in fully reconstructed MR images: proof-of-concept for subsecond position tracking [^115LXveS]. Magnetic Resonance Imaging (2012). Low credibility.
...
### A clearly visible endoscopic instrument shaft on the monitor facilitates hand-eye coordination [^115DqVw7]. Surgical Endoscopy (2002). Low credibility.
...
### A novel theory for rapid localization of the transverse-sigmoid sinus junction and "keyhole" in the retrosigmoid keyhole approach: micro-anatomical study, technique nuances, and clinical application [^1142EYKr]. Neurosurgical Review (2024). Medium credibility.
...
### Heading direction with respect to a reference point modulates place-cell activity [^116xtP6Q]. Nature Communications (2019). High credibility.
...
### Reliability of a preliminary 3-D pain mapping program [^117Lyg2m]. Pain Medicine (2011). Low credibility.
...
### A novel theory for rapid localization of the transverse-sigmoid sinus junction and "keyhole" in the retrosigmoid keyhole approach: micro-anatomical study, technique nuances, and clinical application [^115vjePs]. Neurosurgical Review (2024). Medium credibility.
...
### Self-powered multifunctional sensing based on super-elastic fibers by soluble-core thermal drawing [^111Qe9oW]. Nature Communications (2021). High credibility.
...
### In vivo measurement of the frame-based application accuracy of the neuromate neurosurgical robot [^114g65fJ]. Journal of Neurosurgery (2015). Low credibility.
...
### ACR-ARS practice parameter for image-guided radiation therapy (IGRT) [^115PJjDu]. ARS/ACR (2024). High credibility.
...
### C-arm based navigation in total hip arthroplasty-background and clinical experience [^1141pkxi]. Injury (2004). Low credibility.
...
### ACR-ARS practice parameter for the performance of total body irradiation [^1146TUsC]. ACR/ARS (2022). High credibility.
...
### Human navigation strategies and their errors result from dynamic interactions of spatial uncertainties [^112XVS8Y]. Nature Communications (2024). High credibility.
...
### Goal-oriented representations in the human hippocampus during planning and navigation [^116Tzdk2]. Nature Communications (2023). High credibility.
...
### Spatiotemporal data analysis with chronological networks [^111i5Xwf]. Nature Communications (2020). High credibility.
...
### Benchmarking tools for detecting longitudinal differential expression in proteomics data allows establishing a robust reproducibility optimization regression approach [^1128pJYU]. Nature Communications (2022). High credibility.
...
### ACR-ARS practice parameter for the performance of stereotactic body radiation therapy [^114obUwz]. ARS/ACR (2024). High credibility.
...
### Restoration of complicated epicanthus: modified reverse skin redraping with mini-epicanthoplasty for rescue in unsatisfied epicanthoplasty patients [^116MT9BM]. Annals of Plastic Surgery (2017). Low credibility.
