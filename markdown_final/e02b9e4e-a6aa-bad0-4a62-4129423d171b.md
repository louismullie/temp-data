...
> Next, I should review the general formula and its asymptotics. The exact answer is C(n, ⌊n/2⌋), and by Stirling's approximation this behaves asymptotically as [2ⁿ/√(πn/2)], which illustrates that the largest antichain comprises a substantial fraction of all subsets, roughly on the order of [2ⁿ/√n] [^notfound].
...
> Final answer: The maximum number of elements in such a set A is the binomial coefficient C(n, ⌊n/2⌋), that is, the number of subsets of {1, 2, …, n} of size ⌊n/2⌋. For even n this is C(n, n/2), and for odd n both C(n, (n−1)/2) and C(n, (n+1)/2) achieve the same maximum [^notfound].
