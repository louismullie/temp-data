...
Σ_{k=0 to n} [|A_k|/C(n, k)] ≤ 1,

where (A_k) is the set of subsets in (A) of size (k). Since C(n, k) is maximized at (k = ⌊n/2⌋), the largest possible antichain has size C(n, ⌊n/2⌋), achieved when (A) consists of all subsets of size (⌊n/2⌋).
