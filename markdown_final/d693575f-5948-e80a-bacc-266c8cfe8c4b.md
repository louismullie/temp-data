...
- Removing the last two digits of (N) yields another perfect square, say (m^2).
...
where (r) is the two-digit remainder (i.e. (0 \\leq r \\leq 99)) and (r \\neq 0) (since (N) does not end in zero).
...
- (k^2 = 100 \\cdot m^2 + r) for some integers (m) and (r) with (1 \\leq r \\leq 99).
...
This is a generalized Pell-like equation, but since (r) is small (1–99), we can solve it by testing successive values of (m) and checking whether (100 \\cdot m^2 + r) is a perfect square for some (r) in the valid range.
...
- For (m = 4), (100 \\cdot 4^2 = 1600). Adding (r = 81) gives (1681 = 41^2), which is a perfect square. Removing the last two digits yields 16, which is (4^2).
- For (m = 5), (100 \\cdot 5^2 = 2500). Adding any (r) from 1 to 99 yields 2501–2599, none of which are perfect squares.
- For (m = 6), (100 \\cdot 6^2 = 3600). Adding any (r) from 1 to 99 yields 3601–3699, none of which are perfect squares.
...
In our experiments, the corresponding values of Er ranged from ≃0.5 to 850: for Er<1, the elastic effects outweighed the viscous effects, whereas at Er>1, the viscous effects are dominant over their elastic counterpart. As Re and Er are both proportional to vl, we immediately find that Re= Kρ / η 2 Er. Inserting the typical numbers for the materials properties of LCs, we find a general conversion factor: Re≃10 −4 Er, for K ≃10 −11, obtained as a ratio of interaction energy between molecules and the molecular distance, ρ ≃10 3 and η ≃10 −2 (ref.). Therefore, the specific conversion factor depends on the material properties of the particular LC and can vary for different LC compounds. For 5CB, Re≃10 −6 Er. Nevertheless, the experiments and simulations remain in the laminar flow regime for all Er considered.
