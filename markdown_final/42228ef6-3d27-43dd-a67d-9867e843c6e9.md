...
> I will now examine sensitivity analyses to test robustness, including per-protocol analyses where appropriate, alternative missing data strategies, and checks for influential observations; I should confirm that any deviations from the pre-specified plan are justified transparently and that sensitivity results are interpreted alongside the primary analysis to contextualize stability of findings [^1153fuTk] [^114WZTam].
