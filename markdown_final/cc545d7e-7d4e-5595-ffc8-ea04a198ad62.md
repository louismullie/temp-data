...
### The neural coding framework for learning generative models [^116mpyaN]. Nature Communications (2022). High credibility.
...
### Learning non-stationary langevin dynamics from stochastic observations of latent trajectories [^112HzBFp]. Nature Communications (2021). High credibility.
...
### Interpretable generative deep learning: an illustration with single cell gene expression data [^114khy9v]. Human Genetics (2022). Medium credibility.
...
### Visual motion perception as online hierarchical inference [^115MX4Ec]. Nature Communications (2022). High credibility.
...
### Data-driven identification of tumor subregions based on intravoxel incoherent motion reveals association with proliferative activity [^115uRtt9]. Magnetic Resonance in Medicine (2019). Medium credibility.
...
### Solving the where problem and quantifying geometric variation in neuroanatomy using generative diffeomorphic mapping [^1143HJgU]. Nature Communications (2025). High credibility.
...
### Unsupervised approach to decomposing neural tuning variability [^115EXGfi]. Nature Communications (2023). High credibility.
...
### The neural coding framework for learning generative models [^116DhbBm]. Nature Communications (2022). High credibility.
...
### Model-based prediction of spatial gene expression via generative linear mapping [^112m6Zgh]. Nature Communications (2021). High credibility.
...
### A probabilistic model of biological ageing of the lungs for analysing the effects of smoking, asthma and COPD [^114ALo7x]. Respiratory Research (2013). Low credibility.
...
### Model-based clustering of meta-analytic functional imaging data [^114NjBXT]. Human Brain Mapping (2008). Low credibility.
...
### Exploration-based learning of a stabilizing controller predicts locomotor adaptation [^1166E5Re]. Nature Communications (2024). High credibility.
...
### Adjusting for genetic confounders in transcriptome-wide association studies improves discovery of risk genes of complex traits [^113AJSQy]. Nature Genetics (2024). High credibility.
...
### CloneSig can jointly infer intra-tumor heterogeneity and mutational signature activity in bulk tumor sequencing data [^112DvuHc]. Nature Communications (2021). High credibility.
...
### RA3 is a reference-guided approach for epigenetic characterization of single cells [^117Tbpzc]. Nature Communications (2021). High credibility.
...
### Evaluations of maximization procedures for estimating linkage parameters under heterogeneity [^111yxY6m]. Genetic Epidemiology (2004). Low credibility.
...
### Complexity control by gradient descent in deep networks [^111k3sfM]. Nature Communications (2020). High credibility.
...
### Incorporating genotyping uncertainty in haplotype inference for single-nucleotide polymorphisms [^112jPj8x]. American Journal of Human Genetics (2004). Low credibility.
...
### Integrative differential expression and gene set enrichment analysis using summary statistics for scRNA-seq studies [^115Fc1FG]. Nature Communications (2020). High credibility.
...
### Inferring multimodal latent topics from electronic health records [^114jMs2E]. Nature Communications (2020). High credibility.
...
### Songbirds work around computational complexity by learning song vocabulary independently of sequence [^115v5dRr]. Nature Communications (2017). Medium credibility.
...
### Homopolymer switches mediate adaptive mutability in mismatch repair-deficient colorectal cancer [^1145ERW8]. Nature Genetics (2024). High credibility.
...
### Designing accurate emulators for scientific processes using calibration-driven deep models [^115LXXbo]. Nature Communications (2020). High credibility.
...
### Analyzing atomic force microscopy images of virus-like particles by expectation-maximization [^1117Weo5]. NPJ Vaccines (2024). Medium credibility.
...
### Reinforcement learning establishes a minimal metacognitive process to monitor and control motor learning performance [^112d99vG]. Nature Communications (2023). High credibility.
...
### A machine learning automated recommendation tool for synthetic biology [^114XTDgN]. Nature Communications (2020). High credibility.
...
### Classification of spatiotemporal hemodynamics from brain perfusion MR images using expectation-maximization estimation with finite mixture of multivariate gaussian distributions [^1137z5js]. Magnetic Resonance in Medicine (2007). Low credibility.
...
### Expectation management [^112ghfV5]. The New England Journal of Medicine (2014). Excellent credibility.
...
### Bayesian EM algorithm for scoring polymorphic deletions from SNP data and application to a common CNV on 8q24 [^115sghtE]. Genetic Epidemiology (2009). Low credibility.
...
### Designing accurate emulators for scientific processes using calibration-driven deep models [^114GKwxU]. Nature Communications (2020). High credibility.
...
### Synergizing habits and goals with variational bayes [^116qq4p2]. Nature Communications (2024). High credibility.
...
### A finite mixture model for X-chromosome association with an emphasis on microbiome data analysis [^114T6gce]. Genetic Epidemiology (2019). Medium credibility.
...
### Predictive modeling of heterogeneous treatment effects in RCTs: a scoping review [^112YgBz9]. JAMA Network Open (2025). High credibility.
...
### A latent outcome variable approach for mendelian randomization using the stochastic expectation maximization algorithm [^115ajDws]. Human Genetics (2025). Medium credibility.
...
### Second opinion needed: communicating uncertainty in medical machine learning [^113hq9cm]. NPJ Digital Medicine (2021). Medium credibility.
...
### An introduction to machine learning [^111Uy3Zu]. Clinical Pharmacology and Therapeutics (2020). Medium credibility.
...
### PGlycoQuant with a deep residual network for quantitative glycoproteomics at intact glycopeptide level [^1166PT8C]. Nature Communications (2022). High credibility.
...
### Replicate sequencing libraries are important for quantification of allelic imbalance [^112xb8EB]. Nature Communications (2021). High credibility.
...
### A variational expectation-maximization framework for balanced multi-scale learning of protein and drug interactions [^114WWKKm]. Nature Communications (2024). High credibility.
...
### Artificial intelligence and machine learning for cardiovascular computed tomography (CCT): a white paper of the Society of Cardiovascular Computed Tomography (SCCT) [^116gByg7]. Journal of Cardiovascular Computed Tomography (2024). High credibility.
...
### Algorithms for inferring haplotypes [^113vhWTm]. Genetic Epidemiology (2004). Low credibility.
...
### Scalable spatiotemporal prediction with Bayesian neural fields [^111bVgwM]. Nature Communications (2024). High credibility.
...
### The adaptive designs CONSORT extension (ACE) statement: a checklist with explanation and elaboration guideline for reporting randomised trials that use an adaptive design [^116WLWym]. BMJ (2020). Excellent credibility.
...
### Selecting fitted models under epistemic uncertainty using a stochastic process on quantile functions [^112CmSBC]. Nature Communications (2025). High credibility.
...
### Evolution after whole-genome duplication: a network perspective [^116QzgTR]. G3 (2013). Low credibility.
...
### Cell type and gene expression deconvolution with bayesPrism enables Bayesian integrative analysis across bulk and single-cell RNA sequencing in oncology [^116GZCcr]. Nature Cancer (2022). High credibility.
...
### Single photon emission computed tomography (SPECT) myocardial perfusion imaging guidelines: instrumentation, acquisition, processing, and interpretation [^1125cuBC]. Journal of Nuclear Cardiology (2018). Medium credibility.
...
### Recommendations for the appropriate structure, communication, and investigation of tobacco harm reduction claims. An official American Thoracic Society policy statement [^111aSpFz]. American Journal of Respiratory and Critical Care Medicine (2018). Medium credibility.
...
### Multimodal prediction of conversion to Alzheimer's disease based on incomplete biomarkers [^113zsFxQ]. Alzheimer's & Dementia (2015). Low credibility.
...
### Recommendations for the appropriate structure, communication, and investigation of tobacco harm reduction claims. An official American Thoracic Society policy statement [^112o9kda]. American Journal of Respiratory and Critical Care Medicine (2018). Medium credibility.
...
### Learning non-stationary langevin dynamics from stochastic observations of latent trajectories [^114gULrB]. Nature Communications (2021). High credibility.
...
### A machine learning automated recommendation tool for synthetic biology [^113Lm4Ca]. Nature Communications (2020). High credibility.
...
### A benchmark, expand, and calibration (benchExCal) trial emulation approach for using real-world evidence to support indication expansions: design and process for a planned empirical evaluation [^113FoF9Z]. Clinical Pharmacology and Therapeutics (2025). Medium credibility.
...
### Model-free estimation of completeness, uncertainties, and outliers in atomistic machine learning using information theory [^1163N3At]. Nature Communications (2025). High credibility.
...
### The clinical application of polygenic risk scores: a points to consider statement of the American College of Medical Genetics and genomics (ACMG) [^116PYagF]. Genetics in Medicine (2023). High credibility.
...
### Laboratory perspectives in the development of polygenic risk scores for disease: a points to consider statement of the American College of Medical Genetics and genomics (ACMG) [^115w8nTv]. Genetics in Medicine (2023). High credibility.
...
### Cognitive functioning in untreated glioma patients: the limited predictive value of clinical variables [^112krVkH]. Neuro-Oncology (2024). Medium credibility.
...
### European Respiratory Society guidelines for the diagnosis of primary ciliary dyskinesia [^115bYKfD]. The European Respiratory Journal (2017). Medium credibility.
...
### The adaptive designs CONSORT extension (ACE) statement: a checklist with explanation and elaboration guideline for reporting randomised trials that use an adaptive design [^117Tb5C2]. BMJ (2020). Excellent credibility.
...
### Advancing principled pharmacoepidemiologic research to support regulatory and healthcare decision making: the era of real-world evidence [^115qCe4i]. Clinical Pharmacology and Therapeutics (2025). Medium credibility.
...
### Bayesian MRI reconstruction with joint uncertainty estimation using diffusion models [^1178snKh]. Magnetic Resonance in Medicine (2023). Medium credibility.
...
### A comparison of approaches to account for uncertainty in analysis of imputed genotypes [^11517SST]. Genetic Epidemiology (2011). Low credibility.
...
### Learning non-stationary langevin dynamics from stochastic observations of latent trajectories [^1111ecux]. Nature Communications (2021). High credibility.
...
### Efficient probabilistic inference in generic neural networks trained with non-probabilistic feedback [^113YsfvS]. Nature Communications (2017). Medium credibility.
