...
> Let me first confirm the clinical context. Schizoaffective disorder spans psychotic and affective symptoms, and treatment often requires antipsychotic coverage plus mood stabilization or antidepressant augmentation depending on subtype, which makes antidepressant use common in real-world care even though high-quality schizoaffective-specific trials are sparse, so I need to be careful not to overgeneralize from schizophrenia or bipolar data [^a8bb1979] [^29d26639].
...
> Next, I should review indirect evidence from schizophrenia. Multiple network meta-analyses indicate that adjunctive duloxetine can improve negative symptoms in schizophrenia, with moderate-to-large effect sizes in some analyses, and one analysis even ranked duloxetine among the top augmentation strategies for negative symptoms; again, these are largely single-study contributions within broader networks, so heterogeneity and imprecision apply, but the directionality is consistent across independent syntheses [^4c5159b3] [^8a0aae14] [^8cd45991].
