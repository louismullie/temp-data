...
> I need to ensure I'm not missing special populations or exceptions. The XR food-effect guidance is universal for adults; there are no labeled exceptions for hepatic or renal impairment, though dose adjustments are recommended in those conditions, and the same food principle still applies. In older adults, slower titration and lower starting doses are advised, but again, the food instruction remains unchanged [^2aca7c8d].
...
> But wait, what if the patient is switching formulations or using a generic XR? I should confirm bioequivalence considerations. The XR food-effect data are derived from the reference product and apply to bioequivalent generics; therefore, the same without-food or light-meal instruction should be used regardless of XR manufacturer, assuming bioequivalence [^df5f0038] [^63a0b8a1].
