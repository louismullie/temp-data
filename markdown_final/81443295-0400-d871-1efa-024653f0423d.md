...
> I need to verify the approved indications in this age range. The FDA labeling supports cetirizine for allergic rhinitis and chronic urticaria in children aged 6 months to 5 years, with efficacy extrapolated from adults and supported by pediatric pharmacokinetic and safety data, so use in a 33-month-old for these indications is on-label when dosed correctly [^notfound] [^115jBQv2] [^113tuEQQ].
