...
> Let me first confirm the age mapping. A 33-month-old is 2 years and 9 months, which places the child in the 2 to 5 year pediatric group for cetirizine labeling purposes, not the 6 months to less than 2 years group, so I should apply the 2 to 5 year dosing guidance rather than infant dosing [^117GrANP] [^111tBf6e].

> I need to verify the approved indications in this age range. The FDA labeling supports cetirizine for allergic rhinitis and chronic urticaria in children aged 6 months to 5 years, with efficacy extrapolated from adults and supported by pediatric pharmacokinetic and safety data, so use in a 33-month-old for these indications is on-label when dosed correctly [^112U6AaT] [^115jBQv2] [^113tuEQQ].
