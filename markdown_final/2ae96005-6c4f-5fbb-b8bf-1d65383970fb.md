...
The scanning and reconstruction process is simulated end-to-end in terms of RF events, gradient moment events in x and y, and delay times, acting on the input model spin system given in terms of proton density, T₁ and T₂, and ΔB₀. As a proof of concept, we use both conventional MR images and < mml:math xmlns:mml = "http://www.w3.org/1998/Math/MathML" > < mml:msub > < mml:mi > T < /mml:mi > < mml:mn > 1 < /mml:mn > < /mml:msub > < /mml:math > maps as targets and optimize from scratch using the loss defined by data fidelity, SAR penalty, and scan time.
