...
We consider two polynomial functions: y₁ = a x² − bx + c and y₂ = dx + e. Suppose we want to solve for the coefficients a, b, c, d, and e, which we will take to be positive. As the ground truth coefficients to be determined, we choose (a, b, c, d, e) = (0.5, 3, 5, 1, 1.5).
...
### Cerebral oxygenation during locomotion is modulated by respiration [^114ddvGu]. Nature Communications (2019). High credibility.
...
The number of ways to rank the numbers e 0, e X, e Y and e X+Y was computed using the recurrence relation Aₙᵏ = Aₙ₋₁ᵏ + k Aₙ₋₁ᵏ⁻¹ which, for n = 4 and summing up for k = 1, 2, 3 and 4 gives 75 possible rankings. Each ranking can be seen as a set of outcomes of the following qualitative pairwise comparisons:
e X versus e 0
e Y versus e 0
e X+Y versus e 0
e Y versus e X
...
If a ranking is consistent with a positive (or negative) interaction, the inequality constraints encoded in the corresponding vector can be solved simultaneously with 7a (or 8a). To verify this, we developed a constraint satisfaction that attempts to solve the six constraints of each ranking together with the inequality 7a (or 7b). The solution is searched numerically with a MATLAB linear solver. The variables e 0, e X, e Y and e X+Y were constrained to the interval 2–16, taken as an approximate of the range of expression values from Affymetrix chips in log₂ scale. The method is implemented in the MATLAB code and is available upon request.
...
and the second moments of the change in male and female expressions are approximately E[(Δzₘ)² | zₘ, z_f] = E[(Δz_f)² | zₘ, z_f] = σ², and E[(Δzₘ)(Δz_f) | zₘ, z_f] = ρ σ². For simplicity, we set σ² = 0.1. Setting different values for σ 2 changes the impact of mutations, but since it does so with the same intensity in males and females, it does not change our results. However, increasing σ 2 increases the overall rate of evolution.

---

### Multi-condensate state as a functional strategy to optimize the cell signaling output [^115kLDZH]. Nature Communications (2024). High credibility.

Fig. 2
Reaction-diffusion model with a single condensate.

A Illustration of the simulation geometry. The "Virtual Cell" software is used to define the geometry and set up the simulation conditions. The hemispherical condensate is situated on the XY plane. Arp2/3 only diffuses around the condensate surface. Actins (both G and F) can diffuse in the entire region. B Concentration profiles (shown as color bars) of the model components at the beginning of simulations (Time = 0.2 s, upper panel) and the steady state (Time = 20 s, lower panel). These snapshots represent the plane at Z = 0. Diffusion constants, D_GA = 10 µm²/s and D_FA = 1 µm²/s. C Timecourse of F-actin (FA) concentration at two FA diffusion constants (D FA). To compute the "Surface concentration", a hemispherical region of interest (larger than the condensate) is considered (details in method) and the amount of FA in the shell volume measures the surface concentration. The "Bulk concentration" incorporates all the volume elements, including the condensate surface. D Measure of local production of F-actin as a function of differential diffusion of actin in monomeric (D GA) and polymeric (D FA) form. We keep D_GA (10 µm²/s) fixed and vary D_FA. E Trend of local FA production as a function of the condensate's HemiSphericity.
...
### The dynamics and geometry of choice in the premotor cortex [^116Dxx1D]. Nature (2025). Excellent credibility.
...
### Confronting false discoveries in single-cell differential expression [^116FPGJi]. Nature Communications (2021). High credibility.
...
### The evolution of sex-biased genes and sex-biased gene expression [^1125wMyC]. Nature Reviews: Genetics (2007). Medium credibility.
...
### Balancing false positives and false negatives for the detection of differential expression in malignancies [^115CbxiC]. British Journal of Cancer (2004). Low credibility.
...
### Analysis of multi-condition single-cell data with latent embedding multivariate regression [^112yQdys]. Nature Genetics (2025). High credibility.
...
### X chromosome regulation: diverse patterns in development, tissues and disease [^111v3wqs]. Nature Reviews: Genetics (2014). Medium credibility.
...
### Advancing mathematics by guiding human intuition with AI [^111JmTPD]. Nature (2021). Excellent credibility.
...
### The human Y chromosome, in the light of evolution [^111JRAse]. Nature Reviews: Genetics (2001). Medium credibility.
...
### Edrophonium chloride (Enlon) [^116JGjdi]. FDA (2006). Low credibility.

The dosage of edrophonium chloride IM for differential diagnosis of myasthenia gravis in adults is 10 mg IM once

---

### Balancing false positives and false negatives for the detection of differential expression in malignancies [^111zPtTx]. British Journal of Cancer (2004). Low credibility.
...
### Accelerated estimation and permutation inference for ACE modeling [^114iUE5E]. Human Brain Mapping (2019). Medium credibility.
...
### Confronting false discoveries in single-cell differential expression [^114wzmeK]. Nature Communications (2021). High credibility.
...
### Evaluating the efficacy of intervention [^114qCipU]. Endocrinology and Metabolism Clinics of North America (2003). Low credibility.
...
### A preliminary investigation into the use of edge illumination X-ray phase contrast micro-CT for preclinical imaging [^115xYubm]. Molecular Imaging and Biology (2020). Medium credibility.
...
### Dynamics-dependent density distribution in active suspensions [^111SBDxQ]. Nature Communications (2019). High credibility.
...
### No evidence for sex-differential transcriptomes driving genome-wide sex-differential natural selection [^1175EQgR]. American Journal of Human Genetics (2025). Medium credibility.
...
### SNMMI procedure standard / EANM practice guideline on pediatric [99mTc] Tc-DMSA renal cortical scintigraphy: an update [^1137PecK]. SNMMI (2022). High credibility.
...
### Balancing false positives and false negatives for the detection of differential expression in malignancies [^1151scgr]. British Journal of Cancer (2004). Low credibility.
...
### Addressing sex as a biological variable in preclinical models of lung disease: an official American Thoracic Society research statement [^115hQ3gC]. American Journal of Respiratory and Critical Care Medicine (2025). High credibility.
...
### Assisted differential network analysis for gene expression data [^117WFEqD]. Genetic Epidemiology (2021). Medium credibility.
...
### Statistics for X-chromosome associations [^115zxoQx]. Genetic Epidemiology (2018). Low credibility.
...
### Monozygotic twins reveal germline contribution to allelic expression differences [^1132UGWr]. American Journal of Human Genetics (2008). Low credibility.
...
### Evaluating the consistency of differential expression of microRNA detected in human cancers [^115d7zrC]. Molecular Cancer Therapeutics (2011). Low credibility.
...
### SpookyNet: learning force fields with electronic degrees of freedom and nonlocal effects [^113Mh4f3]. Nature Communications (2021). High credibility.
...
### Diagnostic and prognostic prediction using gene expression profiles in high-dimensional microarray data [^114evwax]. British Journal of Cancer (2003). Low credibility.
...
### Transcriptomic signatures of rare variant impacts across sex and the X chromosome [^115d22x2]. HGG Advances (2025). Medium credibility.
...
### Transcriptomic signatures of rare variant impacts across sex and the X chromosome [^114zcDmt]. HGG Advances (2025). Medium credibility.

The human X chromosome contains hundreds of genes and has well-established impacts on sex differences and traits. However, the X chromosome is often excluded from many genetic analyses, limiting broader understanding of variant effects. In particular, the functional impact of rare variants on the X chromosome is understudied. To investigate functional rare variants on the X chromosome, we use observations of outlier gene expression from Genotype Tissue Expression consortium data. We show that outlier genes are enriched for having nearby rare variants on the X chromosome, and this enrichment is stronger for males. Using the RIVER model, we identified 733 rare variants in 450 genes predicted to have functional differences between males and females. We examined the pharmacogenetic implications of these variants and observed that 25% of drugs with a known sex difference in adverse drug reactions were connected to genes that contained a sex-biased rare variant. We further identify that sex-biased rare variants preferentially impact transcription factors with predicted sex-differential binding, such as the XIST-modulated SIX1. Overall, we observed more within-sex variation than between-sex variation. Combined, our study investigates functional rare variants on the X chromosome, and further details how sex stratification of variant effect prediction improves identification of rare variants with predicted sex-biased effects, transcription factor biology, and pharmacogenomic impacts.

---

### Optimizing differential expression analysis for proteomics data via high-performing rules and ensemble inference [^116WmzTK]. Nature Communications (2024). High credibility.
...
### When to keep it simple-adaptive designs are not always useful [^115oyHay]. BMC Medicine (2019). Medium credibility.
...
### Genome-wide gene expression effects of sex chromosome imprinting in drosophila [^111EaiDf]. G3 (2014). Low credibility.
...
### Acorde unravels functionally interpretable networks of isoform co-usage from single cell data [^1115h3yD]. Nature Communications (2022). High credibility.
...
### Critical non-hermitian skin effect [^114Nit75]. Nature Communications (2020). High credibility.
...
### Animal diversity and ecosystem functioning in dynamic food webs [^1147sqmv]. Nature Communications (2016). Medium credibility.
...
### Addressing sex as a biological variable in preclinical models of lung disease: an official American Thoracic Society research statement [^1123nsHJ]. American Journal of Respiratory and Critical Care Medicine (2025). High credibility.
...
### Surveillance, case investigation and contact tracing for monkeypox [^1177x6GM]. Geneva: World Health Organization (2022). High credibility.
...
### Genome-wide gene expression effects of sex chromosome imprinting in drosophila [^112ZUdDX]. G3 (2014). Low credibility.
...
### Collective magnetism in an artificial 2D XY spin system [^115THS65]. Nature Communications (2018). Medium credibility.
...
### Addressing sex as a biological variable in preclinical models of lung disease: an official American Thoracic Society research statement [^111Svs3q]. American Journal of Respiratory and Critical Care Medicine (2025). High credibility.
...
### Fully-automated and ultra-fast cell-type identification using specific marker combinations from single-cell transcriptomic data [^111QXy3Q]. Nature Communications (2022). High credibility.
...
### Sex chromosome genes directly affect brain sexual differentiation [^115FxD4W]. Nature Neuroscience (2002). Medium credibility.
...
### Pseudo-projection-driven, self-gated cardiac cine imaging using cartesian golden step phase encoding [^113JxyDu]. Magnetic Resonance in Medicine (2016). Low credibility.
...
### Surveillance, case investigation and contact tracing for monkeypox [^115U2tLi]. Geneva: World Health Organization (2022). High credibility.
...
### Genome-wide gene expression effects of sex chromosome imprinting in drosophila [^111xEuoa]. G3 (2014). Low credibility.
...
### A geometric approach to characterize the functional identity of single cells [^1112LWnE]. Nature Communications (2018). Medium credibility.
...
### Evaluating molecular biomarkers for the early detection of lung cancer: when is a biomarker ready for clinical use? An official American Thoracic Society policy statement [^111oQe3R]. American Journal of Respiratory and Critical Care Medicine (2017). Medium credibility.
...
### Monkeypox: preparation and collection of specimens [^112ALhjR]. CDC (2022). High credibility.
...
### Transcriptomic signatures of rare variant impacts across sex and the X chromosome [^111i6GoY]. HGG Advances (2025). Medium credibility.
...
### Evaluating molecular biomarkers for the early detection of lung cancer: when is a biomarker ready for clinical use? An official American Thoracic Society policy statement [^117GKLYf]. American Journal of Respiratory and Critical Care Medicine (2017). Medium credibility.
...
### Dissecting gene expression changes accompanying a ploidy-based phenotypic switch [^116qbb7A]. G3 (2017). Low credibility.
...
### Surveillance, case investigation and contact tracing for monkeypox [^115Sn3rQ]. Geneva: World Health Organization (2022). High credibility.
...
### Novel tools for comparing the architecture of psychopathology between neurogenetic disorders: an application to X-versus Y-chromosome aneuploidy effects in males [^1137Nsin]. Psychological Medicine (2025). Medium credibility.
...
### NCCN guidelines® insights: melanoma: cutaneous, version 2.2024 [^1124XuM2]. Journal of the National Comprehensive Cancer Network (2024). High credibility.
...
### X-chromosome upregulation operates on a gene-by-gene basis at RNA and protein levels [^115K7kC2]. Nature Communications (2025). High credibility.
...
### Diverse and asymmetric patterns of single-neuron projectome in regulating interhemispheric connectivity [^114KYGf9]. Nature Communications (2024). High credibility.
...
### Genes that escape from X inactivation [^1134ueFo]. Human Genetics (2011). Low credibility.
...
### Comprehensive evaluation of deconvolution methods for human brain gene expression [^112x94L4]. Nature Communications (2022). High credibility.
...
### Experimental search for high-temperature ferroelectric perovskites guided by two-step machine learning [^114LNLxe]. Nature Communications (2018). Medium credibility.
...
### Surveillance, case investigation and contact tracing for monkeypox [^113jtHvV]. Geneva: World Health Organization (2022). High credibility.
