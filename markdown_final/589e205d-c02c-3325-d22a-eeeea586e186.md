...
> I will now derive the cumulative distribution function G(z). For z ≤ 0, G(z) = 0. For 0 < z ≤ 2, G(z) = ∫ from 0 to z [t/4] dt = [z²/8]. For 2 < z < 4, I need to add the area up to 2 and then the remaining trapezoid: G(z) = 1/2 + ∫_{2}^{z} (4 − t)/4 dt = 1/2 + [4t/4 − t²/8]_{2}^{z} = 1/2 + (z − z²/8) − (2 − 4/8) = −z²/8 + z − 1. For z ≥ 4, G(z) = 1. Let me verify continuity at z = 2; both pieces give 1/2, so it is continuous, which is expected for a continuous random variable [^notfound].
