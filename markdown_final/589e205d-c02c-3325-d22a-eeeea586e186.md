...
> Now, let me consider the first region, 0 < z ≤ 2. In this case, the overlap of the supports is straightforward: x must be in (0, z) so that z − x is in (0, z) and thus in (0, 2). Wait, let me verify the upper limit; yes, for z ≤ 2, the upper bound is z because z − x must remain positive and less than 2, which is automatically satisfied when z ≤ 2. So g(z) = ∫ from 0 to z [1/2] × [1/2] dx = [z/4] for 0 < z ≤ 2 [^notfound].

> Next, I should review the second region, 2 < z < 4. Here the geometry changes: x must be at least z − 2 so that z − x ≤ 2, and x must be less than 2. So the integral becomes g(z) = ∫ from z − 2 to 2 [1/2] × [1/2] dx = [1/4](2 − (z − 2)) = [1/4](4 − z) for 2 < z < 4. Hold on, I should double-check the algebra; yes, that simplifies to (4 − z)/4, which is a linear decrease from 1/2 at z = 2 to 0 at z = 4 [^notfound].
