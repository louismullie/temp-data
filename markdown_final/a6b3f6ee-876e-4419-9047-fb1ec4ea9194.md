...
> Hmm, wait a minute, I initially wondered if there might be an exception where a full therapeutic dose could be started immediately, but I should double-check that assumption. I find no FDA-approved or guideline-endorsed scenario that permits starting at a full maintenance dose without titration; even in studies using higher starting doses, those were within controlled protocols with close monitoring and still reflected incremental escalation, so the practical, safety-conscious approach remains slow titration for all indications [^notfound].
