...
> Let me first confirm the regulatory status so I don't jump to conclusions. Depakote is FDA-approved for epilepsy, bipolar mania, and migraine prevention; there is no FDA approval for dementia-related agitation or behavioral symptoms, so any use in dementia would be off-label, which already lowers the evidentiary bar I should accept for routine practice [^notfound] [^111tGydh] [^1154grH1].
