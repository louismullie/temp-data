...
> Let me first confirm the fundamental property. In a cyclic quadrilateral, opposite angles are supplementary, so A + C = 180° and B + D = 180°. This is the key constraint that will drive whether each trigonometric identity holds universally.

> Now, relation (1): sin A = sin C. Wait, let me verify this. Since A + C = 180°, we have C = 180° − A, and sin(180° − A) = sin A, so sin C = sin A. Therefore, relation (1) always holds for any cyclic quadrilateral.

> Next, relation (2): sin A + sin C = 0. Hold on, let's not jump to conclusions. From above, sin C = sin A, so sin A + sin C = 2 sin A, which equals zero only when sin A = 0, i.e., when A is 0° or 180°. But angles in a quadrilateral must be strictly between 0° and 180°, so sin A ≠ 0 and this sum is not zero in general. Thus, relation (2) does not always hold.

> I will now examine relation (3): cos B + cos D = 0. Let me think about this carefully. Since B + D = 180°, we have D = 180° − B, and cos(180° − B) = −cos B, so cos D = −cos B, which means cos B + cos D = cos B − cos B = 0. Therefore, relation (3) always holds for any cyclic quadrilateral.

> Finally, relation (4): cos B = cos D. Hmm, wait a minute, I initially thought this might be true because opposite angles are supplementary, but I should double-check. Using D = 180° − B, we get cos D = −cos B, so cos B = cos D would imply cos B = −cos B, which means cos B = 0. That only happens when B is 90°, so equality holds only in special cases, not universally. Thus, relation (4) does not always hold.

> Let me summarize and count. Relations (1) and (3) are always true, whereas relations (2) and (4) are not universally true. Therefore, exactly two relations always hold, and the correct choice is (B) two.
