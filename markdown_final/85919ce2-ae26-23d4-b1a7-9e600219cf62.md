...
The correct answer is **(B) two**. In a cyclic quadrilateral, opposite angles are supplementary, so (A + C = 180°) and (B + D = 180°). This implies:

- **Relation (1)**: (sin A = sin C) always holds because (sin(180° − x) = sin x).
- **Relation (3)**: (cos B + cos D = 0) always holds because (cos(180° − x) = −cos x).
...
Using the supplementary property, (C = 180° − A), so:
...
sin A + sin C = sin A + sin A = 2sin A

This equals zero only if (sin A = 0), which implies (A = 0°) or (180°). These are degenerate cases for a quadrilateral, so **this relation does not generally hold**.

---

### Relation (3): (cos B + cos D = 0)

Using the supplementary property, (D = 180° − B), so:
...
cos B = cos D ⇒ cos B = −cos B ⇒ 2cos B = 0 ⇒ cos B = 0

This requires (B = 90°), which is not generally true. Thus, **this relation does not generally hold**.
...
|-|-|-|
| (1) (sin A = sin C) | Yes | (sin(180° − x) = sin x) |
| (2) (sin A + sin C = 0) | No | Only if (sin A = 0) (degenerate) |
| (3) (cos B + cos D = 0) | Yes | (cos(180° − x) = −cos x) |
