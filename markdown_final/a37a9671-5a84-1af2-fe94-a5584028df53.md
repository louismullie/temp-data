...
> For the essential singularity at (z = 0), I need to check the most straightforward path. Direct Laurent expansion of e^(1/z) around z = 0 gives ∑ n=0..∞ [1/n!] z^(−n), and dividing by (1 - z) would require multiplying two series, which is messy. Hold on, I should verify whether a contour integral approach is cleaner; integrating (f(z)) around a small circle |z| = ε and expanding 1/[1 − z] as a geometric series yields the residue as the coefficient of (z^{-1}) in the product, which is ∑ n=0..∞ [1/n!] = e. Let me double-check that calculation; yes, the coefficient of z^(−1) is indeed the sum of [1/n!] over n ≥ 0, which equals e.
