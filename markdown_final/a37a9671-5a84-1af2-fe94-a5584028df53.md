...
> Now, for the simple pole at (z = 1), I should confirm the residue using the limit formula. The residue is lim_{z→1} (z − 1) f(z) = lim_{z→1} [e^(1/z)/(−1)] = −e, so the residue at (z = 1) is −e. Wait, let me verify the sign; yes, because (1 - z = -(z - 1)), the limit correctly yields (-e).

> For the essential singularity at (z = 0), I need to check the most straightforward path. Direct Laurent expansion of (e^{1/z}) around (z = 0) gives Σ_{n=0}^∞ [1/n!] z^(−n), and dividing by (1 - z) would require multiplying two series, which is messy. Hold on, I should verify whether a contour integral approach is cleaner; integrating (f(z)) around a small circle |z| = ε and expanding 1/[1 − z] as a geometric series yields the residue as the coefficient of (z^{-1}) in the product, which is Σ_{n=0}^∞ [1/n!] = e. Let me double-check that calculation; yes, the coefficient of (z^{-1}) is indeed the sum of (\\frac{1}{n!}) over (n \\geq 0), which equals (e).
...
The function f(z) = e^(1/z)/[1 − z] has two isolated singularities: a **simple pole at z = 1** and an **essential singularity at z = 0**. The residue at the simple pole is found using the limit formula: Res(f, 1) = lim_{z→1} (z − 1) f(z) = −e. At the essential singularity, we expand e^(1/z) and 1/[1 − z] into Laurent series, multiply them, and extract the coefficient of z^(−1), which gives Res(f, 0) = e − 1. Thus, the residues are Res(f, 1) = −e and Res(f, 0) = e − 1.
...
Res(f, 1) = lim_{z→1} (z − 1) e^(1/z)/[1 − z] = lim_{z→1} (−e^(1/z)) = −e
...
f(z) = (Σ_{n=0}^∞ [1/n!] z^(−n)) (Σ_{m=0}^∞ z^m) = Σ_{k=−∞}^{∞} c_k z^k
