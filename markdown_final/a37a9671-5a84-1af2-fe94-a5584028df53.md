...
> Let's see… What do we have here? The user is asking to find the residues of the function (f(z) = \\frac{e^{1/z}}{1 - z}) at its singular points. Let's break this down step-by-step. First, I need to think about identifying all singular points and classifying them. Then, I should verify the residue at the simple pole. Next, I will examine the essential singularity at zero and decide on the most efficient way to compute its residue. Finally, I will synthesize the results and double-check consistency with standard methods.
...
> Now, for the simple pole at (z = 1), I should confirm the residue using the limit formula. The residue is (\\lim_{z \\to 1} (z - 1) f(z) = \\lim_{z \\to 1} \\frac{e^{1/z}}{-1} = -e), so the residue at (z = 1) is (-e). Wait, let me verify the sign; yes, because (1 - z = -(z - 1)), the limit correctly yields (-e).

> For the essential singularity at (z = 0), I need to check the most straightforward path. Direct Laurent expansion of (e^{1/z}) around (z = 0) gives (\\sum_{n = 0}^{\\infty} \\frac{1}{n!} z^{-n}), and dividing by (1 - z) would require multiplying two series, which is messy. Hold on, I should verify whether a contour integral approach is cleaner; integrating (f(z)) around a small circle (|z| = \\epsilon) and expanding (\\frac{1}{1 - z}) as a geometric series yields the residue as the coefficient of (z^{-1}) in the product, which is (\\sum_{n = 0}^{\\infty} \\frac{1}{n!} = e). Let me double-check that calculation; yes, the coefficient of (z^{-1}) is indeed the sum of (\\frac{1}{n!}) over (n \\geq 0), which equals (e).

> But wait, what if I tried to use the residue at infinity to sidestep the essential singularity? I initially thought I could compute (-\\text{Res}(f, \\infty)) and relate it to residues at finite poles, but hold on, let's not jump to conclusions; that approach works when the sum of finite residues equals minus the residue at infinity, which requires the function to be meromorphic on the extended complex plane. Here, (f(z)) has an essential singularity at zero, so the residue theorem for the extended plane does not apply in the same way, and I should not rely on that shortcut. I need to ensure I stick with the Laurent-series or contour-integral method for the essential singularity, which confirms the residue at (z = 0) is (e).

> Final answer: The residue at the simple pole (z = 1) is (-e), and the residue at the essential singularity (z = 0) is (e). I should confirm the signs one more time; both checks are consistent, so the residues are (\\text{Res}(f, 1) = -e) and (\\text{Res}(f, 0) = e).

---

The function (f(z) = \\frac{e^{1/z}}{1 - z}) has two isolated singularities: a **simple pole at (z = 1)** and an **essential singularity at (z = 0)**. The residue at the simple pole is found using the limit formula: (\\text{Res}(f, 1) = \\lim_{z \\to 1} (z - 1) f(z) = -e). At the essential singularity, we expand (e^{1/z}) and (\\frac{1}{1 - z}) into Laurent series, multiply them, and extract the coefficient of (z^{-1}), which gives (\\text{Res}(f, 0) = e - 1). Thus, the residues are (\\text{Res}(f, 1) = -e) and (\\text{Res}(f, 0) = e - 1).

---

## Singularities of the function

The function (f(z) = \\frac{e^{1/z}}{1 - z}) has singularities where the denominator is zero or where the exponent (1/z) is undefined. These occur at (z = 1) (a simple pole) and (z = 0) (an essential singularity).
...
[
\\text{Res}(f, 1) = \\lim_{z \\to 1} (z - 1) \\frac{e^{1/z}}{1 - z} = \\lim_{z \\to 1} -e^{1/z} = -e
]
...
At an essential singularity, we find the residue from the Laurent series expansion of (f(z)) around (z = 0). We expand (e^{1/z}) and (\\frac{1}{1 - z}) and multiply them to identify the coefficient of (z^{-1}).
...
[
f(z) = \\left(\\sum_{n = 0}^{\\infty} \\frac{1}{n!} z^{-n} \\right) \\left(\\sum_{m = 0}^{\\infty} z^m \\right) = \\sum_{k = -\\infty}^{\\infty} c_k z^k
]

The coefficient of (z^{-1}) is:

[
c_{-1} = \\sum_{n = 1}^{\\infty} \\frac{1}{n!} = e - 1
]
...
The residues of (f(z) = \\frac{e^{1/z}}{1 - z}) are (\\text{Res}(f, 1) = -e) and (\\text{Res}(f, 0) = e - 1).
...
The 8 events of (x, y, z) having nonzero probability in b (1, λ) are those in which z lies in row y and column x, as shown in this grid. The probabilities of these events arefor even values of x,for odd values of x, and. In other words, b (1, λ) consists of four blocks of uniform probability, each corresponding to a different value of z. Within each block, X and Y are perfectly correlated, but with a nonuniform distribution (λ, 1 − λ). For example, the event (X = 1, Y = 2, Z = 3) occurs with probability 1/8 according to distribution b (1, λ), while the event (X = 1, Y = 2, Z = 2) occurs with probability zero.
...
To implement 3D magnetic resonance fingerprinting (MRF) with quadratic RF phase (qRF-MRF) for simultaneous quantification of T 1 , T 2 , ΔB 0 , and <mml:math xmlns:mml="http://www.w3.org/1998/Math/MathML"><mml:msubsup><mml:mi>T</mml:mi><mml:mn>2</mml:mn><mml:mo>∗</mml:mo></mml:msubsup></mml:math> .

Methods

3D MRF data with effective undersampling factor of 3 in the slice direction were acquired with quadratic RF phase patterns for T 1 , T 2 , and <mml:math xmlns:mml="http://www.w3.org/1998/Math/MathML"><mml:msubsup><mml:mi>T</mml:mi><mml:mn>2</mml:mn><mml:mo>∗</mml:mo></mml:msubsup></mml:math> sensitivity. Quadratic RF phase encodes the off-resonance by modulating the on-resonance frequency linearly in time. Transition to 3D brings practical limitations for reconstruction and dictionary matching because of increased data and dictionary sizes. Randomized singular value decomposition (rSVD)-based compression in time and reduction in dictionary size with a quadratic interpolation method are combined to be able to process prohibitively large data sets in feasible reconstruction and matching times.

Results

Accuracy of 3D qRF-MRF maps in various resolutions and orientations are compared to 3D fast imaging with steady-state precession (FISP) for T 1 and T 2 contrast and to 2D qRF-MRF for <mml:math xmlns:mml="http://www.w3.org/1998/Math/MathML"><mml:msubsup><mml:mi>T</mml:mi><mml:mn>2</mml:mn><mml:mo>∗</mml:mo></mml:msubsup></mml:math> contrast and ΔB 0 . The precision of 3D qRF-MRF was 1.5-2 times higher than routine clinical scans. 3D qRF-MRF ΔB 0 maps were further processed to highlight the susceptibility contrast.

Conclusion

Natively co-registered 3D whole brain T 1 , T 2 , <mml:math xmlns:mml="http://www.w3.org/1998/Math/MathML"><mml:msubsup><mml:mi>T</mml:mi><mml:mn>2</mml:mn><mml:mo>∗</mml:mo></mml:msubsup></mml:math> , ΔB 0 , and QSM maps can be acquired in as short as 5 min with 3D qRF-MRF.
