...
> Hold on, I should verify the total sum constraint. Summing xₖ from k = 1 to n gives ∑_{k = 1}ⁿ [c/(1 − c)ᵏ] = 1. This is a geometric series with first term [c/(1 − c)] and ratio [1/(1 − c)], so the sum equals [c/(1 − c)] × [((1 − c)^(−n) − 1)/((1 − c)^(−1) − 1)] = [c/(1 − c)] × [((1 − c)^(−n) − 1)/([c/(1 − c)])] = (1 − c)^(−n) − 1. Setting this equal to 1, I get (1 − c)^(−n) − 1 = 1, so (1 − c)^(−n) = 2, which implies 1 − c = 2^(−1/n) and therefore c = 1 − 2^(−1/n).
...
The smallest possible value of S is 1 − 2^(−1/n), achieved when xₖ = 2^((k−1)/n) (2^(1/n) − 1) for k = 1, 2, …, n. This configuration equalizes all the fractions [xₖ/(1 + ∑_{i = 1}ᵏ xᵢ)] to 1 − 2^(−1/n), which is the minimal achievable maximum.
...
We minimize S = max_{1 ≤ k ≤ n} [xₖ/(1 + ∑_{i = 1}ᵏ xᵢ)] subject to ∑_{i = 1}ⁿ xᵢ = 1 and xᵢ > 0. The Lagrangian is:

L = max_{1 ≤ k ≤ n} [xₖ/(1 + ∑_{i = 1}ᵏ xᵢ)] − λ (∑_{i = 1}ⁿ xᵢ − 1)
...
∑_{k = 1}ⁿ xₖ = (2^(1/n) − 1) ∑_{k = 1}ⁿ 2^((k−1)/n) = (2^(1/n) − 1) × [(2 − 1)/(2^(1/n) − 1)] = 1
