...
> Let me first confirm the setup. We have x₁, x₂, …, xₙ > 0 with ∑(i=1..n) xᵢ = 1, and we define S = max over 1 ≤ k ≤ n [xₖ/(1 + ∑_{i = 1}ᵏ xᵢ)]. The goal is to minimize S over all such sequences, and to find the minimizing sequence if it exists.
