...
Find the smallest possible value of $S$. For what values of $x_{1}, x_{2}, \cdots, x_{n}$ is this minimum value achieved?
...
> Final answer: The smallest possible value of (S) is (1–2^{-1/n}), and this minimum is achieved when (x_k = (1–2^{-1/n}) \\cdot 2^{k/n}) for (k = 1, 2, \\dots, n). Let me just double-check that all (x_k) are positive and sum to 1, which they are and do, confirming the solution is valid.

---

The smallest possible value of (S) is (1–2^{-1/n}), achieved when (x_k = 2^{(k-1)/n} (2^{1/n} - 1)) for (k = 1, 2, \\dots, n). This configuration equalizes all the fractions (\\frac{x_k}{1 + \\sum_{i = 1}^k x_i}) to (1–2^{-1/n}), which is the minimal achievable maximum.
...
For (n = 1), we have (x_1 = 1), so (S = \\frac{x_1}{1 + x_1} = \\frac{1}{2} = 1–2^{-1/1}). The formula holds.
...
[
\\mathcal{L} = \\max_{1 \\leq k \\leq n} \\frac{x_k}{1 + \\sum_{i = 1}^k x_i} - \\lambda \\left(\\sum_{i = 1}^n x_i - 1 \\right)
]

At the optimum, all fractions are equal to some constant (c):

[
\\frac{x_k}{1 + \\sum_{i = 1}^k x_i} = c \\quad \\text{for all } k
]

This gives the recurrence:

[
x_k = c \\left(1 + \\sum_{i = 1}^k x_i \\right)
]
...
[
\\sum_{k = 1}^n c (1 - c)^{-k} = 1
]

This simplifies to:

[
c \\cdot \\frac{(1 - c)^{-n} - 1}{c} = 1 \\implies (1 - c)^{-n} = 2
]
...
[
x_k = (1–2^{-1/n}) \\cdot 2^{k/n} = 2^{(k-1)/n} (2^{1/n} - 1)
]
...
[
\\sum_{k = 1}^n x_k = (2^{1/n} - 1) \\sum_{k = 1}^n 2^{(k-1)/n} = (2^{1/n} - 1) \\cdot \\frac{2–1}{2^{1/n} - 1} = 1
]

And for each (k):

[
\\frac{x_k}{1 + \\sum_{i = 1}^k x_i} = \\frac{2^{(k-1)/n} (2^{1/n} - 1)}{1 + (2^{k/n} - 1)} = 1–2^{-1/n}
]

---

Thus, the smallest possible value of (S) is (1–2^{-1/n}), achieved when (x_k = 2^{(k-1)/n} (2^{1/n} - 1)) for (k = 1, 2, \\dots, n).
...
Compute stage 1 pooled testing results: • if v j > LOD then pool j tests positive, where the LOD (limit of detection) we use is 100. • otherwise, pool j tests negative with probability 0.99 (i.e. the false-positive rate of PCR results is 1%).
...
Record the following for the current trial r and day d : • the number of true positive individuals identified by HYPER:, • the number of tests expended: T (r) (d) = m + number of tests used in stage 2, • the number of true positive individuals seen:= number of individuals with viral load > 0.
...
### Algorithms for the survival output rate table (...) [^d8294b98]. seer.cancer.gov (2025). Medium credibility.

\ \ \. Relative Survival Section of Output Table \ where
- \= each individual's expected probability for surviving interval. x.
- The sum is over all individuals entering interval xalive. \_i \). where \ refers to the expected probability of each individual for surviving intervals 1, 2, ., x and the sum is over all individuals in the table. \ \ \}}\).
