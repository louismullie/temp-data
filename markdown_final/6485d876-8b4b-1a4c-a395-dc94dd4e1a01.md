...
Figure 2
Model 1 of side-view measurement of CCT representing Scheimpflug-geometry tomographic imaging of the cornea. The variable d s is the apparent corneal thickness in the Scheimpflug image, angle θ is the constant observation angle (45°), and r is the known radius of curvature of the anterior cornea. The unknowns are the index of refraction of the cornea, n c and corrected CCT side view, t cs. CCT = central corneal thickness.

Based on triangle BGO in Figure 2, with angles, θi, (180−θ), and angle α, which can be defined by (α = θ – θi), the following equation results.where θi is the angle of incidence of a parallel light ray at point B, d s is observed apparent corneal thickness in the image, angle θ is the constant observation angle (45°), and r is the known radius of curvature of the anterior cornea. Based on Snell's Law, angle θ r is defined by the following equation.where n c is the index of refraction of the cornea.
...
We used the Pythagorean equation (c² = a² + b²) to calculate the adequate shaft length of the forceps to use in macular surgery, especially in highly myopic eyes, where c² represents the axial length (hypotenuse); b² the sum of the corneal ray and distance between the corneal limbus and the sclerotomy (base side); and a² the distance between the sclerotomy and the fovea (perpendicular side).
...
2.1 B₁⁺ mapping with multi‐spin‐echo sequences
...
Despite the compact forms of Equations 1 and 2, it is not obvious how the B₁⁺ amplitude can be retrieved from the echo amplitudes of CP and CPMG echo trains. Using a generating functions formalism, Lukzen et al, showed that the asymptotic behavior of Equations 1 and 2 is of the following form:
