...
> But wait, what if a patient is already using neem and asks whether to continue? I need to ensure shared decision-making: given the lack of direct efficacy data for acne, I would discuss stopping neem in favor of evidence-based regimens, while monitoring for irritation or allergic reactions; if the patient prefers to continue, I would position it as unproven adjunctive care alongside guideline-supported therapy and document the off-label use with counseling on expected timelines and safety [^115VPeci] [^111sXWYh].
