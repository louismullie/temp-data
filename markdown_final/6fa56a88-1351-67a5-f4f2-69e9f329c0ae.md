...
> I should double-check the formal reasoning. The ratio simplifies to (100a + 10b + c)/(a + b + c). For fixed a, increasing b or c increases the denominator faster than the numerator relative to the existing sum, so the ratio decreases. Therefore, the maximum for a given a occurs when b = c = 0, yielding 100a/a = 100. Since a ≤ 9, the global maximum is achieved when a = 9, b = 0, c = 0, giving 100 exactly.

> Let me reconsider edge cases to ensure completeness. The smallest three-digit number is 100, which gives 100/1 = 100, but wait, that's the same maximum value, not larger. However, the problem asks for the maximum, and while 100 achieves the same ratio, 900 is the largest three-digit number that attains it, so 900/9 = 100 is the canonical answer. No three-digit number produces a ratio greater than 100, confirming the result.

> Final answer: The maximum value of the ratio of a three-digit number to the sum of its digits is 100, achieved by the number 900.
