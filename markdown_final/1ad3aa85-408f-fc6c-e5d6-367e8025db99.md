...
> Let me first confirm the line's parametric representation. The symmetric form (\\frac{x-1}{-2} = \\frac{y-2}{1} = \\frac{z+1}{-1}) implies a direction vector (\\vec{d} = (-2, 1, -1)) and a point on the line (P_0 = (1, 2, -1)), so the parametric equations are (x = 1–2t), (y = 2 + t), and (z = -1 - t), where (t) is a real parameter [^notfound].
...
[
\\frac{x - 1}{-2} = t \\implies x = 1–2t
]

[
\\frac{y - 2}{1} = t \\implies y = 2 + t
]

[
\\frac{z + 1}{-1} = t \\implies z = -1 - t
]

So the parametric form is:

[
x = 1–2t, \\quad y = 2 + t, \\quad z = -1 - t
]
...
[
9–9t = 0 \\implies 9t = 9 \\implies t = 1
]
...
The embryos were recovered after laser irradiation, grown until the L4 stage, and placed singly in drops of M9+5 μg ml −1 DiI for 30 min (ref.). The animals were recovered onto plates and imaged with wide-field microscopy.
...
We provide several components for display and interaction purposes that are useful for obtaining input for various registration and segmentation tasks:,,, and. All components allow the user to zoom and pan the displayed images as part of the supported interactions.
