...
> Let me first confirm the line's parametric representation. The symmetric form ([(x − 1)/−2] = [(y − 2)/1] = [(z + 1)/−1]) implies a direction vector (d = (−2, 1, −1)) and a point on the line (P₀ = (1, 2, −1)), so the parametric equations are (x = 1–2t), (y = 2 + t), and (z = -1 - t), where (t) is a real parameter.

> Wait, let me verify the plane equation before substituting. The plane is given as (x - 2y + 5z + 17 = 0), so I should substitute the parametric expressions for (x), (y), and (z) into this equation to find the value of (t) at intersection.

> I will now substitute the parametric forms into the plane equation: ((1–2t) - 2(2 + t) + 5(-1 - t) + 17 = 0). Expanding this gives (1–2t - 4–2t - 5–5t + 17 = 0), which simplifies to (-9t + 9 = 0), so (t = 1). Hold on, I should double-check the arithmetic: (1–4–5 + 17 = 9) and (-2t - 2t - 5t = -9t), so yes, (t = 1) is correct.

> Next, I should review the back-substitution to get the intersection point. Plugging (t = 1) into the parametric equations yields (x = 1–2(1) = -1), (y = 2 + 1 = 3), and (z = -1–1 = -2), so the intersection point is ((-1, 3, -2)).

> I need to ensure this point satisfies both the line and the plane. Let me first confirm it lies on the line: substituting into the symmetric form gives ([(-1 − 1)/−2] = [(3 − 2)/1] = [(-2 + 1)/−1]), which simplifies to (1 = 1 = 1), so the point is on the line. Now, I should confirm it lies on the plane: ((-1) - 2(3) + 5(-2) + 17 = -1–6–10 + 17 = 0), which checks out. Hmm, wait a minute, I initially thought the plane check might fail, but the math confirms the point satisfies the plane equation as well.
...
### Quenched disorder and instability control dynamic fracture in three dimensions [^113QCaMt]. Nature Communications (2024). High credibility.
...
### Kinetic features dictate sensorimotor alignment in the superior colliculus [^113SLFYE]. Nature (2024). Excellent credibility.
...
### Sensitivity and spectral control of network lasers [^115SPXTG]. Nature Communications (2022). High credibility.
...
The real axis is an invariant subspace, retrieving our real results; yet complexification endows the dynamics with an intriguing capability: as Fig. 4a, b illustrates through the (x, y) phase plane, collapse is avoided in the presence of a minuscule imaginary part. Large elliptical-looking trajectories are traced on the phase plane, eventually returning to the neighborhood of the sole fixed point of (0, 0) — which in the real case one would characterize as semi-stable. The system of Eq. (19) can be tackled in closed form since the ODEyields [1/z] = − t + [1/z(0)]. For z = x + iy (z (0) = x 0 + iy 0) we obtain the explicit orbit formulaEliminating time by dividing the two ODEs within Eq. (19) directly yields an ODE for y = y (x) (rather than the parametric forms of Eqs. (20), (21)). From this ODE, one can obtain that the quantityis an invariant of the phase plane dynamics, and thus the latter can be written as x² + (y − R)² = R², where. That is, the trajectory evolves along circles of radius R in the upper (resp. lower) half plane if y₀ > 0 (resp. y₀ < 0.) Approaching the axis with y₀ → 0, the curvature of these circles tends to 0 and their radius to ∞ (retrieving the real dynamics as a special case). Figure 4 through its planar projections illustrates not only the radial projection of the dynamics in the x − y plane, but the x − t and y − t dependencies.

---

### Superconductivity and a van hove singularity confined to the surface of a topological semimetal [^116wTuA6]. Nature Communications (2025). High credibility.
...
### Easy-to-actuate multi-compatible Truss structures with prescribed reconfiguration [^115WcBYV]. Nature Communications (2024). High credibility.
...
### A new method for calculating femoral anterior cortex point location and its effect on component sizing and placement [^1142Td29]. Clinical Orthopaedics and Related Research (2015). Low credibility.
...
### The trochlear sulcus of the native knee is consistently orientated close to the sagittal plane despite variation in distal condylar anatomy [^117PGDMj]. Knee Surgery, Sports Traumatology, Arthroscopy (2023). Medium credibility.
...
### Low polyethylene creep and wear following mobile-bearing unicompartmental knee replacement [^114km2vv]. Knee Surgery, Sports Traumatology, Arthroscopy (2021). Medium credibility.
...
### Spatially expandable fiber-based probes as a multifunctional deep brain interface [^1133mwx7]. Nature Communications (2020). High credibility.
...
### Engineering phase and polarization singularity sheets [^116W4r39]. Nature Communications (2021). High credibility.
...
### Caliper-based precise positioning of the target (CALIPPOT) for transcranial magnetic stimulation without neuronavigation system [^1116pwiy]. General Psychiatry (2024). Medium credibility.
...
### Iobenguane I-123 (Adreview) [^1117duAd]. FDA (2024). Medium credibility.
...
### Multiple tipping points and optimal repairing in interacting networks [^115RaNXK]. Nature Communications (2016). Medium credibility.
...
### Advanced statistics: linear regression, part I: simple linear regression [^111cnf44]. Academic Emergency Medicine (2004). Low credibility.
...
### Relative projective location of three bottom apexes of petrous bone on skull [^113MoJZv]. The Journal of Craniofacial Surgery (2016). Low credibility.
...
### A structural transition in physical networks [^114UpQDu]. Nature (2018). Excellent credibility.
...
### 3D multimodal cardiac data reconstruction using angiography and computerized tomographic angiography registration [^114SLUV5]. Journal of Cardiothoracic Surgery (2015). Low credibility.
...
### Direction of actin flow dictates integrin LFA-1 orientation during leukocyte migration [^116jdRYm]. Nature Communications (2017). Medium credibility.
...
### Three-dimensional wave breaking [^113GLSEM]. Nature (2024). Excellent credibility.
...
### Multiple tipping points and optimal repairing in interacting networks [^115tpXRX]. Nature Communications (2016). Medium credibility.
...
### 3D multimodal cardiac data reconstruction using angiography and computerized tomographic angiography registration [^116gev3G]. Journal of Cardiothoracic Surgery (2015). Low credibility.
...
### Study on the anatomic relationship between the clavicle and the coracoid process using computed tomography scans of the shoulder [^116AF5WX]. Journal of Shoulder and Elbow Surgery (2017). Low credibility.
...
### Three-dimensional analysis of bone morphology of the rheumatoid arthritis elbow [^1167RZCS]. Journal of the American Academy of Orthopaedic Surgeons: Global Research & Reviews (2025). Medium credibility.
...
### In situ X-ray diffraction and the evolution of polarization during the growth of ferroelectric superlattices [^116vLmGi]. Nature Communications (2015). Medium credibility.
...
### Bony landmarks in the endoscopic endonasal transoculomotor approach [^111SdH7R]. Neurosurgical Review (2021). Medium credibility.
...
### Tirzepatide (Zepbound) [^115kZtRi]. FDA (2025). Medium credibility.
...
### Samarium hexaboride is a trivial surface conductor [^113t3HAY]. Nature Communications (2018). Medium credibility.
...
### Correlation of contrast sensitivity at low spatial frequencies with myopic shift in Chinese children [^111HaAQb]. BMC Ophthalmology (2025). Medium credibility.
...
### Foot deformity correction planning in the sagittal plane based on the vitruvian foot first metatarsal mechanical axis and calcaneus anatomic axis [^114AbFJc]. The Journal of Foot and Ankle Surgery (2021). Medium credibility.
...
### Selective neural representation of objects relevant for navigation [^113GUFCu]. Nature Neuroscience (2004). Medium credibility.
...
### Multiple tipping points and optimal repairing in interacting networks [^113Dhc8s]. Nature Communications (2016). Medium credibility.
...
### Charge density wave induced nodal lines in LaTe [^116Devyh]. Nature Communications (2023). High credibility.
...
In Fig. 4, we provide an example of a simple power transformation using the square function, which raises a variable to the power of 2 (x²), providing a U-shaped curve. Higher powers of x can be also used to determine the shape of the function, such as the cubic function (x³) which has a distinctive S-shape. These power terms form the core elements of so-called polynomial functions, which along with the coefficients included in the function, offer a flexible way to model various curves. By adjusting the coefficients and power functions, polynomials offer a wide variety of shapes. "Fractional" polynomial functions allow the power terms to be fractions instead of just whole numbers (i.e. x½) [21]. A (fractional) polynomial function often provides sufficient flexibility to follow relatively simple non-linear curves, providing a simpler solution than the more advanced techniques we describe below. However, higher degree polynomials can be sensitive to "noise" in the data, and are not suited for fitting some of the more complex curves (e.g. sudden shifts, discontinuities, or logarithmic curves). They may also suffer from Runge's phenomenon, becoming unstable and oscillating at the edges of the data, and extrapolate poorly beyond the original range of the independent variable.
...
### Articulation points in complex networks [^112LbKr3]. Nature Communications (2017). Medium credibility.
...
### Engineering phase and polarization singularity sheets [^112MqCca]. Nature Communications (2021). High credibility.
...
### Advanced optical analysis of divergence between the foci of the neodymium-doped yttrium aluminum garnet (Nd: YAG) and aiming beam lasers [^112b6fnx]. Ophthalmology Science (2024). Medium credibility.
...
### Atlas (C1) lateral mass screw placement using the intersection between lateral mass and inferomedial edge of the posterior arch: a cadaveric study [^117EbmqW]. European Spine Journal (2022). Medium credibility.
...
### Multiple tipping points and optimal repairing in interacting networks [^112gCwio]. Nature Communications (2016). Medium credibility.
...
### Ultrasound-guided extraforaminal thoracic nerve root block through the midpoint of the inferior articular process and the parietal pleura: a clinical application of thoracic paravertebral nerve block [^115GkMRa]. Journal of Pain Research (2022). Medium credibility.
...
### Creating, moving and merging dirac points with a fermi gas in a tunable honeycomb lattice [^114VVjoh]. Nature (2012). Excellent credibility.
...
### Assumptions of statistical tests: what lies beneath [^116X7sqV]. The Journal of Foot and Ankle Surgery (2017). Low credibility.
...
### Point singularity array with metasurfaces [^114A34mG]. Nature Communications (2023). High credibility.
...
### Sources of path integration error in young and aging humans [^117Wx3yC]. Nature Communications (2020). High credibility.
...
### 3D atomic structure from a single X-ray free electron laser pulse [^116KX2aS]. Nature Communications (2024). High credibility.
...
### Biomarkers of severity and threshold of allergic reactions during oral peanut challenges [^113CurFB]. The Journal of Allergy and Clinical Immunology (2020). Medium credibility.
...
### Atomic structure and domain wall pinning in samarium-cobalt-based permanent magnets [^113ppKoD]. Nature Communications (2017). Medium credibility.
...
### Assessing and improving reliability of neighbor embedding methods: a map-continuity perspective [^115NyRNQ]. Nature Communications (2025). High credibility.
...
### Transient power-law behaviour following induction distinguishes between competing models of stochastic gene expression [^115Zvyn4]. Nature Communications (2025). High credibility.
...
### Correlation coefficients: appropriate use and interpretation [^1114UP4r]. Anesthesia and Analgesia (2018). Low credibility.
...
### Dynamic spatiotemporal beams that combine two independent and controllable orbital-angular-momenta using multiple optical-frequency-comb lines [^115YiGet]. Nature Communications (2020). High credibility.
...
### Uncomputability of phase diagrams [^116wSHv4]. Nature Communications (2021). High credibility.
...
### MRI features of intersection syndrome of the forearm [^117Tz2cK]. AJR: American Journal of Roentgenology (2003). Low credibility.
...
### SimpleITK image-analysis notebooks: a collaborative environment for education and reproducible research [^114ZosXd]. Journal of Digital Imaging (2018). Low credibility.
...
### Spirometry in occupational health-2020 [^117Q7Y5D]. Journal of Occupational and Environmental Medicine (2020). High credibility.
