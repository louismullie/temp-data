...
### Bayesian prediction intervals for assessing P-value variability in prospective replication studies [^1111DSce]. Translational Psychiatry (2017). Low credibility.
...
### Between-batch pharmacokinetic variability inflates type I error rate in conventional bioequivalence trials: a randomized advair diskus clinical trial [^115gz58x]. Clinical Pharmacology and Therapeutics (2017). Low credibility.
...
### Evolving use of ancestry, ethnicity, and race in genetics research – A survey spanning seven decades [^114Ehg2y]. American Journal of Human Genetics (2021). Medium credibility.
...
### Intermodal consistency of whole-brain connectivity and signal propagation delays [^112ox7vW]. Human Brain Mapping (2025). Medium credibility.
...
### Prediction of unfavorable outcome of acute decompensation of diabetes mellitus [^113UY9x9]. Diabetology & Metabolic Syndrome (2025). Medium credibility.
...
### Bayesian prediction intervals for assessing P-value variability in prospective replication studies [^116Lc5zr]. Translational Psychiatry (2017). Low credibility.
...
### Bayesian prediction intervals for assessing P-value variability in prospective replication studies [^117Rrpwd]. Translational Psychiatry (2017). Low credibility.
...
### Selecting fitted models under epistemic uncertainty using a stochastic process on quantile functions [^111HwgaU]. Nature Communications (2025). High credibility.
...
### Bayesian prediction intervals for assessing P-value variability in prospective replication studies [^113aVaVp]. Translational Psychiatry (2017). Low credibility.
...
### Bayesian prediction intervals for assessing P-value variability in prospective replication studies [^115vmCjd]. Translational Psychiatry (2017). Low credibility.
...
### Statistical significance versus clinical importance of observed effect sizes: what Do P values and confidence intervals really represent? [^112iqNHq]. Anesthesia and Analgesia (2018). Low credibility.
...
### Random laser ablated tags for anticounterfeiting purposes and towards physically unclonable functions [^116mDrjm]. Nature Communications (2024). High credibility.
...
### New evidence for positive selection helps explain the paternal age effect observed in achondroplasia [^112kb9Ac]. Human Molecular Genetics (2013). Low credibility.
...
### Statistics 101 for radiologists [^1172dJe4]. Radiographics (2015). Low credibility.
...
### Microbial coexistence through chemical-mediated interactions [^112d8ps8]. Nature Communications (2019). High credibility.
...
### Experimental demonstration of quantum advantage for NP verification with limited information [^111hgJ6x]. Nature Communications (2021). High credibility.
...
### Statistical significance versus clinical importance of observed effect sizes: what Do P values and confidence intervals really represent? [^1153Bp1j]. Anesthesia and Analgesia (2018). Low credibility.
...
### Joint, multifaceted genomic analysis enables diagnosis of diverse, ultra-rare monogenic presentations [^117Ak9N7]. Nature Communications (2025). High credibility.
...
### Time esophageal pH [^115eqUNN]. BMC Gastroenterology (2008). Low credibility.
...
### Serotonin signaling regulates actomyosin contractility during morphogenesis in evolutionarily divergent lineages [^115RFFNG]. Nature Communications (2023). High credibility.
...
### Mapping the probability of schistosomiasis and associated uncertainty, west Africa [^116ajAYx]. Emerging Infectious Diseases (2008). Low credibility.
...
### Population coding of conditional probability distributions in dorsal premotor cortex [^112cmBM1]. Nature Communications (2018). Medium credibility.
...
### Essential information on surgical heart valve characteristics for optimal valve prosthesis selection: expert consensus document from the European Association for Cardio-thoracic Surgery (EACTS)-the Society of Thoracic Surgeons (STS)-American Association for Thoracic Surgery (AATS) valve labelling task force [^114GTuZo]. The Journal of Thoracic and Cardiovascular Surgery (2021). High credibility.

Prosthesis–patient mismatch (PPM) risk estimation — standardized indexed effective orifice area (iEOA) cut-offs and probability method: In standardized PPM charts, the following PPM cut-off values should be used for severe PPM — non-obese (BMI < 30kg/m²): indexed EOA ≤ 0.65 cm²/m²; obese (BMI ≥ 30kg/m²): indexed EOA ≤ 0.55 cm²/m². Expected indexed EOA values follow the same distribution as reference EOA values, and the exact probability of PPM can be calculated by defining the area under the curve below the PPM limit relative to the area under the curve of the whole expected indexed EOA distribution; in standardized PPM charts, the probability of PPM should be provided using this method, which is more useful for clinical decision-making than a 'yes/no' classification.

---

### Data-sharing and trustworthiness of trials evaluating cervical ripening in induction of labour: a meta-epidemiological study of randomised controlled trials [^1127b9dV]. EClinicalMedicine (2025). Medium credibility.
...
### Individual heritable differences result in unique cell lymphocyte receptor repertoires of naïve and antigen-experienced cells [^114Dvi7n]. Nature Communications (2016). Medium credibility.
...
### A computational method for cell type-specific expression quantitative trait loci mapping using bulk RNA-seq data [^113Qvzoi]. Nature Communications (2023). High credibility.
...
### Evaluating the utility of identity-by-descent segment numbers for relatedness inference via information theory and classification [^111cxfNj]. G3 (2022). Medium credibility.
...
### Quantum-secure covert communication on bosonic channels [^1124pfgf]. Nature Communications (2015). Medium credibility.
...
### Variation in mutational (co) variances [^115hwmnb]. G3 (2023). Medium credibility.
...
### Reciprocity of social influence [^114AVkqo]. Nature Communications (2018). Medium credibility.
...
### Estimation-uncertainty affects decisions with and without learning opportunities [^1155jHdJ]. Nature Communications (2025). High credibility.
...
### Mortality rates among hospitalized patients with COVID-19 infection treated with tocilizumab and corticosteroids: a Bayesian reanalysis of a previous meta-analysis [^117KSqRr]. JAMA Network Open (2022). High credibility.
...
### Determining clinical course of diffuse large B-cell lymphoma using targeted transcriptome and machine learning algorithms [^112sHJDT]. Blood Cancer Journal (2022). Medium credibility.
...
### Consistent estimation in mendelian randomization with some invalid instruments using a weighted median estimator [^117Y9kms]. Genetic Epidemiology (2016). Low credibility.
...
### Characteristics of baseline frequency data in spinal RCTs do not suggest widespread non-random allocation [^112t2DQW]. European Spine Journal (2023). Medium credibility.
...
### Daily forecasting of regional epidemics of coronavirus disease with Bayesian uncertainty quantification, United States [^113xCxV6]. Emerging Infectious Diseases (2021). Medium credibility.
...
### A recurrent network model of planning explains hippocampal replay and human behavior [^111SuFid]. Nature Neuroscience (2024). High credibility.
...
### Development of the probability of return of spontaneous circulation in intervals without chest compressions during out-of-hospital cardiac arrest: an observational study [^114o7NKC]. BMC Medicine (2009). Low credibility.
...
### Investigating suspected cancer clusters and responding to community concerns: guidelines from CDC and the council of state and territorial epidemiologists [^115eGTtg]. MMWR: Recommendations and Reports (2013). Medium credibility.
...
### Calculating the probability of random sampling for continuous variables in submitted or published randomised controlled trials [^111Nq6oy]. Anaesthesia (2015). Low credibility.
...
### The net chance of a longer survival as a patient-oriented measure of treatment benefit in randomized clinical trials [^116hRT7A]. JAMA Oncology (2016). Medium credibility.
...
### Selecting fitted models under epistemic uncertainty using a stochastic process on quantile functions [^114GAk8h]. Nature Communications (2025). High credibility.
...
### Uncertainty of risk estimates from clinical prediction models: rationale, challenges, and approaches [^117SwvfZ]. BMJ (2025). Excellent credibility.
...
### Effect sizes in ongoing randomized controlled critical care trials [^1119BfCv]. Critical Care (2017). Low credibility.
...
### Future increase in compound soil drought-heat extremes exacerbated by vegetation greening [^112bp1SN]. Nature Communications (2024). High credibility.
...
### Gaze-centered gating, reactivation, and reevaluation of economic value in orbitofrontal cortex [^113A49Gc]. Nature Communications (2024). High credibility.
...
### Evaluation of clinical prediction models (part 3): calculating the sample size required for an external validation study [^112zFpMe]. BMJ (2024). Excellent credibility.
...
### Computational processes of simultaneous learning of stochasticity and volatility in humans [^115X6gQj]. Nature Communications (2024). High credibility.
...
### Guidance on community viral load: a family of measures, definitions, and method for calculation [^115uWzQS]. CDC (2011). Medium credibility.
