...
[
\\frac{d^n y}{dx^n} = (-1)^{n+1} \\frac{2^n (n-1)!}{\\ln 10 \\cdot (2x + 7)^n}
]
...
[
\\frac{dy}{dx} = \\frac{2}{\\ln 10 \\cdot (2x + 7)}
]
...
The comparison between the exact value and the prediction with Λ = 1 (full expansion) is shown in Fig. 3 a. We select 25 test examples evenly distributed across the interval [− π, π]. By increasing the number of training examples n from 10 to 20, the prediction values of the proposed ML model almost match the exact results.
