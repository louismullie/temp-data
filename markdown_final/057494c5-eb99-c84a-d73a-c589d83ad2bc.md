...
> Let me first confirm the definitions. λ1 is the gas–gas mean free path, which depends on the number density of gas molecules and the collision cross-section between neutral gas molecules; at a given temperature, λ1 is inversely proportional to pressure, so at < 10⁻⁹ Torr, λ1 is very long, on the order of kilometers, which is why ultra-high vacuum is used to minimize gas scattering in TEM [^notfound]. Wait, let me verify that scale: yes, for air at room temperature, λ1 is roughly 1 km at 10⁻⁹ Torr, so the exact value is large but the key point is that λ1 is determined by neutral–neutral collisions and scales inversely with pressure.
