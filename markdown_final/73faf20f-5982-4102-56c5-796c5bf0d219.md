...
> But wait, what if the patient has a penicillin allergy or other modifiers? I need to ensure I address alternatives. For severe beta-lactam allergy, doxycycline or a respiratory fluoroquinolone can be used, though macrolides are less reliable due to resistance and should be reserved when options are limited; in children, amoxicillin remains first-line with dosing adjustments, and observation with close follow-up is reasonable for persistent nonsevere disease given the high spontaneous recovery rate [^114cefYD] [^115VLjBL] [^112hmkXY].
