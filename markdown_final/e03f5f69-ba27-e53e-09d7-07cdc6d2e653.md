...
Here, (yᵢ − ŷᵢ) is the **vertical distance** (residual) between the observed value yᵢ and the predicted value ŷᵢ on the regression line. Squaring these residuals ensures that positive and negative errors do not cancel out and that larger errors are penalized more heavily [^113o1ogh].

---

## Geometric interpretation

Geometrically, OLS finds the line that minimizes the **sum of the squares of the vertical distances** from each data point to the regression line. These vertical distances are the residuals, and squaring them emphasizes larger deviations, making the estimator sensitive to outliers [^113o1ogh].

---

## Why not horizontal distances?

OLS does not minimize horizontal distances because the regression model treats the dependent variable (y) as a function of the independent variable (x), with errors assumed to occur in the vertical direction (i.e. in the measurement of (y)). Minimizing horizontal distances would imply errors in (x), which is not the standard assumption in OLS regression [^111cnf44].

---

## Common misconceptions

- **Option A**: "OLS minimises the sum of the vertical distances from the points to the line" is incorrect because it omits the squaring of residuals, which is central to the OLS method [^113o1ogh].
- **Option C**: "OLS minimises the sum of the horizontal distances from the points to the line" is incorrect because OLS focuses on vertical residuals, not horizontal deviations [^111cnf44].
- **Option D**: "OLS minimises the sum of the squares of the horizontal distances from the points to the line" is incorrect for the same reason as Option C, and it also incorrectly includes squaring of horizontal distances [^113o1ogh].

---

## Conclusion

OLS regression minimizes the **sum of the squares of the vertical distances** (residuals) from the observed data points to the fitted regression line. This approach ensures that the regression line provides the best linear fit to the data by minimizing the overall squared error, making Option B the correct answer [^113o1ogh].

---

## References

### Dependency and health utilities in stroke: data to inform cost-effectiveness analyses [^112zBz3z]. European Stroke Journal (2017). Low credibility.
...
### Phylogenetically informed predictions outperform predictive equations in real and simulated data [^113o1ogh]. Nature Communications (2025). High credibility.
...
### Predicting prognosis for adults with depression using individual symptom data: a comparison of modelling approaches [^112uX3sn]. Psychological Medicine (2023). Medium credibility.
...
### Effects of safe childbirth checklist on quality of care and birth outcomes in Indonesia… [^116iPdu4]. JAMA Network (2021). Excellent credibility.
...
### Ridge regression in prediction problems: automatic choice of the ridge parameter [^1134YE9b]. Genetic Epidemiology (2013). Low credibility.
...
### Depression, violence and socioeconomic outcomes among refugees in east Africa: evidence from a multicountry representative survey [^114WXMzy]. BMJ Mental Health (2023). High credibility.
...
### Local opinion leaders: effects on professional practice and healthcare outcomes [^1129fay8]. The Cochrane Database of Systematic Reviews (2019). Medium credibility.
...
### Bias due to participant overlap in two-sample mendelian randomization [^112PCLwk]. Genetic Epidemiology (2016). Low credibility.
...
### Improving genetic prediction by leveraging genetic correlations among human diseases and traits [^114dcktY]. Nature Communications (2018). Medium credibility.
...
### Emergent constraints on carbon budgets as a function of global warming [^111vKSMB]. Nature Communications (2024). High credibility.
...
### How the internet increases modern contraception uptake: evidence from eight sub-saharan African countries [^113RQWYF]. BMJ Global Health (2020). High credibility.
...
### Olsalazine sodium (Dipentum) [^114Z34Lx]. FDA (2009). Low credibility.
...
### Refining penalized ridge regression: a novel method for optimizing the regularization parameter in genomic prediction [^1121z9Zg]. G3 (2024). Medium credibility.
...
### Lost in translation: on the problem of data coding in penalized whole genome regression with interactions [^114h9S4e]. G3 (2019). Medium credibility.
...
### Resource allocation for maximizing prediction accuracy and genetic gain of genomic selection in plant breeding: a simulation experiment [^11744rm9]. G3 (2013). Low credibility.
...
### Olsalazine sodium (Dipentum) [^1166w8Qp]. FDA (2009). Low credibility.
...
### High-yield parallel fabrication of quantum-dot monolayer single-electron devices displaying coulomb staircase, contacted by graphene [^115z9zoa]. Nature Communications (2021). High credibility.
...
### Validation and characterisation of a DNA methylation alcohol biomarker across the life course [^112dr193]. Clinical Epigenetics (2019). Medium credibility.
...
### High-yield parallel fabrication of quantum-dot monolayer single-electron devices displaying coulomb staircase, contacted by graphene [^111cM7wk]. Nature Communications (2021). High credibility.
...
### Cross-validation without doing cross-validation in genome-enabled prediction [^116w2WcV]. G3 (2016). Low credibility.
...
### 4-year results from the RAPID-PsA phase 3 randomised placebo-controlled trial of certolizumab pegol in psoriatic arthritis [^115N6AQ9]. RMD Open (2018). Low credibility.
...
### Effect of alcohol label designs with different pictorial representations of alcohol content and health warnings on knowledge and understanding of low-risk drinking guidelines: a randomized controlled trial [^112x1FZL]. Addiction (2021). Medium credibility.
...
### A method to estimate the contribution of rare coding variants to complex trait heritability [^113FozyW]. Nature Communications (2024). High credibility.
...
### Asymmetric one-pot transformation of isoflavones to pterocarpans and its application in phytoalexin synthesis [^1151gNAV]. Nature Communications (2020). High credibility.
...
### Patterns of social connection among older adults in england… [^112nCTKn]. JAMA Network (2024). Excellent credibility.
...
### Enantiopure synthesis of [5] helicene based molecular lemniscates and their use in chiroptical materials [^11592LbB]. Nature Communications (2025). High credibility.
...
### Association between birth weight and refractive error in adulthood: a mendelian randomisation study [^111vxZUP]. The British Journal of Ophthalmology (2020). Medium credibility.
...
### An occupational health intervention programme for workers at high risk for sickness absence. cost effectiveness analysis based on a randomised controlled trial [^115Kkmjm]. Occupational and Environmental Medicine (2008). Low credibility.
...
### FW: an R package for finlay-wilkinson regression that incorporates genomic / pedigree information and covariance structures between environments [^117Sr3ry]. G3 (2015). Low credibility.
...
### Structural equation models in occupational health: an application to exposure modelling [^115k1Gm1]. Occupational and Environmental Medicine (2012). Low credibility.
...
### Built structures influence patterns of energy demand and COemissions across countries [^112imfe3]. Nature Communications (2023). High credibility.
...
### Accelerated estimation and permutation inference for ACE modeling [^113ZY8fs]. Human Brain Mapping (2019). Medium credibility.
...
### An encompassing mendelian randomization study of the causes and consequences of major depressive disorder [^113CwrkS]. Nature: Mental Health (2025). High credibility.
...
### Optimisation of positive end-expiratory pressure by forced oscillation technique in a lavage model of acute lung injury [^116ghfp1]. Intensive Care Medicine (2011). Low credibility.
...
### Mendelian randomization analysis with multiple genetic variants using summarized data [^115RuuJW]. Genetic Epidemiology (2013). Low credibility.
...
### Oligodendrocyte fate after spinal cord injury [^111dcmvb]. Neurotherapeutics (2011). Low credibility.
...
### Pan-cancer multi-omic model of LINE-1 activity reveals locus heterogeneity of retrotransposition efficiency [^113uiHKR]. Nature Communications (2025). High credibility.
...
### Quantile regression in the field of liver transplantation: a case study-based tutorial [^115197rm]. Liver Transplantation (2025). Medium credibility.
...
### A multicentre, prospective, randomised, double-blind study to measure the treatment effectiveness of abobotulinum A (aboBTXA) among women with refractory interstitial cystitis / bladder pain syndrome [^112qpmjE]. International Urogynecology Journal (2014). Low credibility.
...
### Uncovering oligodendrocyte enhancers that control cnp expression [^111Synw2]. Human Molecular Genetics (2023). Medium credibility.
...
### Genetic taste blindness to bitter and body composition in childhood: a mendelian randomization design [^111Jtc36]. International Journal of Obesity (2014). Low credibility.
...
### Are local public expenditure reductions associated with increases in inequality in emergency hospitalisation? Time-series analysis of english local authorities from 2010 to 2017 [^1172MiM4]. Emergency Medicine Journal (2024). Medium credibility.
...
### Prevalence of drug-drug interactions in children with cardiac disorders receiving off-label drugs in a single centre in Pakistan: a cross-sectional study [^112brWXb]. BMJ Paediatrics Open (2025). High credibility.
...
### Is there a causal role for homocysteine concentration in blood pressure? A mendelian randomization study [^115SWFqW]. The American Journal of Clinical Nutrition (2016). Low credibility.
...
### Does higher education protect against obesity? Evidence using mendelian randomization [^113qAxpG]. Preventive Medicine (2017). Low credibility.
...
### Estimating the "impact" of out-of-home placement on child well-being: approaching the problem of selection bias [^1175aimh]. Child Development (2009). Low credibility.
...
### Glial cell deficits are a key feature of schizophrenia: implications for neuronal circuit maintenance and histological differentiation from classical neurodegeneration [^114XQVLR]. Molecular Psychiatry (2025). Medium credibility.
...
### Comparison of direct and indirect methods of estimating health state utilities for resource allocation: review and empirical analysis [^114pKb95]. BMJ (2009). Excellent credibility.
...
### Olmesartan medoxomil (Benicar) [^114kM6xf]. FDA (2023). Medium credibility.
...
### Quality initiative in rectal cancer strategy: a qualitative study of participating surgeons [^1177daWn]. Journal of the American College of Surgeons (2006). Low credibility.
...
### When can group level clustering be ignored? Multilevel models versus single-level models with sparse data [^1159y3sk]. Journal of Epidemiology and Community Health (2008). Low credibility.
...
### Improved prediction of rates of visual field loss in glaucoma using empirical bayes estimates of slopes of change [^113Ziafn]. Journal of Glaucoma (2012). Low credibility.
...
### Regression: understanding what covariates and confounds Do in adjusted analyses [^111ptPmg]. The Journal of Clinical Psychiatry (2024). Medium credibility.
...
### Quantification of neighborhood-level social determinants of health in the continental United States [^115g7QYa]. JAMA Network Open (2020). High credibility.
...
### Oligodendroglial excitability mediated by glutamatergic inputs and nav1.2 activation [^1136tD8o]. Nature Communications (2017). Medium credibility.
...
### Limitations of ordinary least squares models in analyzing repeated measures data [^112EX2Pe]. Medicine and Science in Sports and Exercise (2004). Low credibility.
...
### Repeated measures designs and analysis of longitudinal data: If at first you Do not succeed-try, try again [^1136QgCu]. Anesthesia and Analgesia (2018). Low credibility.
...
### Postpandemic recovery of case mix index and risk-adjusted mortality in US hospitals [^114tr3Ee]. JAMA Network Open (2025). High credibility.
...
### Educational attainment and mental health outcomes: a within-sibship mendelian randomization study [^114y1gfw]. Psychological Medicine (2025). Medium credibility.
...
### The burden of proof studies: assessing the evidence of risk [^112KPXkr]. Nature Medicine (2022). Excellent credibility.
...
### Dicer1 and miR-219 are required for normal oligodendrocyte differentiation and myelination [^113WAhuK]. Neuron (2010). Low credibility.
...
### Dissecting the effect of workplace exposures on workers' rating of psychological health and safety [^1178Pq1B]. American Journal of Industrial Medicine (2019). Medium credibility.
...
### Clinical management update of oral leukoplakia: a review from the American head and neck society cancer prevention service [^11147QvX]. Head & Neck (2025). Medium credibility.
...
### Healthcare expenditures associated with depression among individuals with osteoarthritis: post-regression linear decomposition approach [^1115Dw8u]. Journal of General Internal Medicine (2015). Low credibility.
...
### Counterfactual reasoning underlies the learning of priors in decision making [^111AhAxt]. Neuron (2018). Low credibility.
...
### Comparison of direct and indirect methods of estimating health state utilities for resource allocation: review and empirical analysis [^115NHDrW]. BMJ (2009). Excellent credibility.
...
### Calcium, magnesium, potassium, and sodium oxybates (Xywav) [^11438nRw]. FDA (2025). Medium credibility.
...
### US Cystic Fibrosis Foundation and European Cystic Fibrosis Society consensus recommendations for the management of non-tuberculous mycobacteria in individuals with cystic fibrosis [^1124GKUA]. Thorax (2016). Medium credibility.
...
### Compared with what? Estimating the effects of injury prevention policies using the synthetic control method [^116wqwuG]. Injury Prevention (2018). Low credibility.
...
### Low measles seropositivity in vaccinated children-JAMA network… [^114upDRk]. JAMA Network (2025). Excellent credibility.
...
### Oligodendrocyte dynamics in the healthy adult CNS: evidence for myelin remodeling [^114fWhJf]. Neuron (2013). Low credibility.
...
### Biased expectations about future choice options predict sequential economic decisions [^112VafRf]. Communications Psychology (2024). Medium credibility.
...
### Morphological and functional convergence of visual projection neurons from diverse neurogenic origins in drosophila [^117G7feE]. Nature Communications (2025). High credibility.
...
### Biased expectations about future choice options predict sequential economic decisions [^114K9ZhP]. Communications Psychology (2024). Medium credibility.
...
### Age-related anabolic resistance of myofibrillar protein synthesis is exacerbated in obese inactive individuals [^115Ymvnd]. The Journal of Clinical Endocrinology and Metabolism (2017). Low credibility.
...
### Primer on binary logistic regression [^114b7D96]. Family Medicine and Community Health (2021). Medium credibility.
...
### Evidence-based guideline: premature ovarian insufficiency [^113CL3W9]. Fertility and Sterility (2025). High credibility.
...
### Using multilevel models and generalized estimating equation models to account for clustering in neurology clinical research [^115rk9V8]. Neurology (2024). Medium credibility.
...
### Biased expectations about future choice options predict sequential economic decisions [^113HgLxU]. Communications Psychology (2024). Medium credibility.
...
### The fly connectome reveals a path to the effectome [^114YMXbm]. Nature (2024). Excellent credibility.
...
### Characterizing oligodendrocyte-lineage cells and myelination in the basolateral amygdala: insights from a novel methodology in postmortem human brain [^113BsJ64]. Psychiatry Research (2025). Medium credibility.
...
### Statistical predictions with glmnet [^1165AYWe]. Clinical Epigenetics (2019). Medium credibility.
...
### Methods for obtaining unpublished data [^114gr4t1]. The Cochrane Database of Systematic Reviews (2011). Low credibility.
...
### Performance of linear mixed models in estimating structural rates of glaucoma progression using varied random effect distributions [^116bEAhb]. Ophthalmology Science (2023). Medium credibility.
...
### Adaptation and initial results of the Polish version of the GA (2) LEN chronic urticaria quality of life questionnaire (CU-Q (2) oL) [^114S2ejB]. Journal of Dermatological Science (2011). Low credibility.
