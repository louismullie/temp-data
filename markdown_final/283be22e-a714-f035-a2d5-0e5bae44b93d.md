...
The equation (√[x − √(1 − x)] + √x = 1) has a unique solution: **(x = [16/25])**. To solve it, isolate one radical, square both sides, and repeat until the radicals are eliminated. Then solve the resulting polynomial and verify the solution against the original equation and domain constraints. The domain requires (0 \\leq x \\leq 1) and (x \\geq \\sqrt{1 - x}), which (x = \\frac{16}{25}) satisfies.
