...
The integer solutions to (x³ + 1 = y²) are:

- ((x, y) = (-1, 0))
- ((x, y) = (0, ± 1))
- ((x, y) = (2, ± 3))

These are the only integer solutions, as confirmed by algebraic factorization and verification against known results for Mordell's equation (y² = x³ + k).
...
For (y²) to be a perfect square, the product ((x + 1)(x² − x + 1)) must be a perfect square. Let (d = gcd(x + 1, x² − x + 1)). By the Euclidean algorithm, (d) divides 3, so (d ∈ {1, 3}).

---

### Case 1: (d = 1)

If (d = 1), then (x + 1) and (x² − x + 1) are coprime. For their product to be a square, each factor must be a square (up to sign). Since (x² − x + 1 > 0) for all real (x), both factors must be non-negative squares. Thus:

x + 1 = a² and x² − x + 1 = b²

for some non-negative integers (a, b). Substitute (x = a²−1) into the second equation:

(a²–1)² − (a²–1) + 1 = b²

a⁴−3a² + 3 = b²

For (a = 0), (x = -1) and (y = 0), giving the solution ((-1, 0)). For (a = 1), (x = 0) and (y = ± 1), giving ((0, ± 1)). For (a = 2), (x = 3) and (b² = 7), which is not a square. For (a ≥ 3), (a⁴−3a² + 3) lies strictly between consecutive squares, so no further solutions exist in this case.

---

### Case 2: (d = 3)

If (d = 3), then (x + 1 = 3k) and (x² − x + 1 = 3m) for integers (k, m) with (gcd(k, m) = 1). The product becomes:

9km = y² ⇒ km is a square

Since (k) and (m) are coprime, both must be squares. Let (k = u²) and (m = v²), so:

x + 1 = 3u² and x² − x + 1 = 3v²

Substitute (x = 3u²−1) into the second equation:
...
For (u = 1), (x = 2) and (v² = 1), so (y = ± 3), giving ((2, ± 3)). For (u ≥ 2), (3u⁴−3u² + 1) lies strictly between consecutive squares, so no further solutions exist in this case.
...
- **For (x = -1)**: ((−1)³ + 1 = 0 = 0²), so ((-1, 0)) is valid.
- **For (x = 0)**: (0³ + 1 = 1 = (± 1)²), so ((0, ± 1)) are valid.
- **For (x = 2)**: (2³ + 1 = 9 = (± 3)²), so ((2, ± 3)) are valid.

---

## Conclusion

The complete set of integer solutions to (x³ + 1 = y²) is:
...
### Evolving scientific discovery by unifying data and background knowledge with AI hilbert [^113D3UDd]. Nature Communications (2024). High credibility.
...
### Pixel super-resolution with spatially entangled photons [^111ZoSXr]. Nature Communications (2022). High credibility.
...
### Regularized joint water-fat separation with bmap estimation in image space for 2D-navigated interleaved EPI based diffusion MRI [^112AEs8j]. Magnetic Resonance in Medicine (2021). Medium credibility.
...
### Verification test problems cardiac electrophysiology… [^111zZDZy]. FDA (2025). Medium credibility.
...
### Reconfigurable metamaterial processing units that solve arbitrary linear calculus equations [^111YFa99]. Nature Communications (2024). High credibility.
...
### Cubic regression-based degree of correction predicts the performance of whole bisulfitome amplified DNA methylation analysis [^116vwHGa]. Epigenetics (2012). Low credibility.
...
### Cell invasion during competitive growth of polycrystalline solidification patterns [^114LSZDv]. Nature Communications (2023). High credibility.
...
### Efficient generative modeling of protein sequences using simple autoregressive models [^112SmXJm]. Nature Communications (2021). High credibility.
...
### Microbial community-level regulation explains soil carbon responses to long-term litter manipulations [^114Y4n9S]. Nature Communications (2017). Medium credibility.
...
### A first-principle mechanism for particulate aggregation and self-assembly in stratified fluids [^114swYHC]. Nature Communications (2019). High credibility.
...
### Methane emissions from the nord stream subsea pipeline leaks [^1168TJip]. Nature (2025). Excellent credibility.
...
### Modeling X-linked ancestral origins in multiparental populations [^1157XKLM]. G3 (2015). Low credibility.
...
### Adaptive resetting for informed search strategies and the design of non-equilibrium steady-states [^115iyyvV]. Nature Communications (2025). High credibility.
...
### The mechanics of landslide mobility with erosion [^114wq41D]. Nature Communications (2021). High credibility.
...
### Genotypic frequencies at equilibrium for polysomic inheritance under double-reduction [^112SV9ct]. G3 (2019). Medium credibility.
...
### Analytical realization of complex thermal meta-devices [^112kbFEg]. Nature Communications (2024). High credibility.
...
### Haplotype-based genome-wide prediction models exploit local epistatic interactions among markers [^116QBWaX]. G3 (2018). Low credibility.
...
### NBTS meeting full transcript… [^116KYJWB]. FDA (2025). Medium credibility.
...
### A general derivation and quantification of the third law of thermodynamics [^1111pqp1]. Nature Communications (2017). Medium credibility.
...
### Conserved linear dynamics of single-molecule brownian motion [^113awub2]. Nature Communications (2017). Medium credibility.
...
### A universal description of ultraslow glass dynamics [^112uZsXE]. Nature Communications (2013). Medium credibility.
...
### Quantum engine efficiency bound beyond the second law of thermodynamics [^116RErv2]. Nature Communications (2018). Medium credibility.
...
### Robust 3D bloch-siegert basedmapping using multi-echo general linear modeling [^1117n5XB]. Magnetic Resonance in Medicine (2019). Medium credibility.

Purpose

Quantitative MRI applications, such as mapping the T 1 time of tissue, puts high demands on the accuracy and precision of transmit field (B₁⁺) estimation. A candidate approach to satisfy these requirements exploits the difference in phase induced by the Bloch-Siegert frequency shift (BSS) of 2 acquisitions with opposite off-resonance frequency radiofrequency pulses. Interleaving these radiofrequency pulses ensures robustness to motion and scanner drifts; however, here we demonstrate that doing so also introduces a bias in the B₁⁺ estimates.

Theory and Methods

It is shown here by means of simulation and experiments that the amplitude of the error depends on MR pulse sequence parameters, such as repetition time and radiofrequency spoiling increment, but more problematically, on the intrinsic properties, T 1 and T 2, of the investigated tissue. To solve these problems, a new approach to BSS-based B₁⁺ estimation that uses a multi-echo acquisition and a general linear model to estimate the correct BSS-induced phase is presented.
...
The general linear model-based method is recommended as a more accurate approach to BSS-based B₁⁺ mapping.
