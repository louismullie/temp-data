...
For (y^2) to be a perfect square, the product ((x + 1)(x^2 - x + 1)) must be a perfect square. Let (d = gcd(x + 1, x² − x + 1)). By the Euclidean algorithm, (d) divides 3, so (d ∈ {1, 3}).
...
For (u = 1), (x = 2) and (v² = 1), so (y = ± 3), giving ((2, ± 3)). For (u ≥ 2), (3u^4–3u^2 + 1) lies strictly between consecutive squares, so no further solutions exist in this case.
