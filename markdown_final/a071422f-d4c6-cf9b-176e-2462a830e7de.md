...
Example 2 Given $x < \frac{5}{4}$, find the maximum value of the function $f(x) = 4 x-2+$ $\frac{1}{4 x-5}$.
...
Setting (g'(u) = 0) gives (1 - \\frac{1}{u^2} = 0), so (u^2 = 1). Since (u < 0), the critical point is (u = -1).
...
[
4x - 5 = -1 \\implies 4x = 4 \\implies x = 1
]

So, the maximum occurs at (x = 1), which satisfies (x < \\frac{5}{4}).

---

## Conclusion

The maximum value of (f(x) = 4x - 2 + \\frac{1}{4x - 5}) for (x < \\frac{5}{4}) is **1**, achieved at (x = 1).
