...
10 × 84 Given that OA and OB are two different lines, take any point on $O A$ and draw a perpendicular to $O B$, then from this foot of the perpendicular draw a perpendicular to $O A$, and from the second foot of the perpendicular draw a perpendicular to $O B$, and so on to infinity. If the lengths of the first two perpendicular segments are a, b (a ≠ b), then the sum of these perpendicular segments, as the number of perpendiculars increases indefinitely, approaches a limit. This limit value is
(A) $\frac{b}{a-b}$.
(B) $\frac{a}{a-b}$.
(C) $\frac{a b}{a-b}$.
(D) $\frac{b^{2}}{a-b}$.
...
In general, the lengths alternate between (a cos^(2n) θ) and (b cos^(2n) θ) for (n = 0, 1, 2, …).
