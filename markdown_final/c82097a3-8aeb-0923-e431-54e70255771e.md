...
$10 \cdot 84$ Given that $O A$ and $O B$ are two different lines, take any point on $O A$ and draw a perpendicular to $O B$, then from this foot of the perpendicular draw a perpendicular to $O A$, and from the second foot of the perpendicular draw a perpendicular to $O B$, and so on to infinity. If the lengths of the first two perpendicular segments are $a, b(a \neq b)$, then the sum of these perpendicular segments, as the number of perpendiculars increases indefinitely, approaches a limit. This limit value is
(A) $\frac{b}{a-b}$.
(B) $\frac{a}{a-b}$.
(C) $\frac{a b}{a-b}$.
(D) $\frac{b^{2}}{a-b}$.
...
The limit of the sum of the perpendicular segments is a geometric series with first term (a) and common ratio (\\frac{b}{a}), so the sum is (\\frac{a}{1 - \\frac{b}{a}} = \\frac{a^2}{a - b}). The correct answer is **(E) (\\frac{a^2}{a - b})**.

---

## Geometric setup and sequence of lengths

Let the angle between lines (OA) and (OB) be (\\theta). The construction generates a sequence of perpendiculars whose lengths form a geometric sequence:

- **First perpendicular from (OA) to (OB)**: length (a).
- **Second perpendicular from (OB) to (OA)**: length (b).
- **Third perpendicular from (OA) to (OB)**: length (a \\cos^2 \\theta).
- **Fourth perpendicular from (OB) to (OA)**: length (b \\cos^2 \\theta).

In general, the lengths alternate between (a \\cos^{2n} \\theta) and (b \\cos^{2n} \\theta) for (n = 0, 1, 2, \\dots).
...
[
S = a + b + a \\cos^2 \\theta + b \\cos^2 \\theta + a \\cos^4 \\theta + b \\cos^4 \\theta + \\cdots
]

Factor this into two geometric series:

[
S = a (1 + \\cos^2 \\theta + \\cos^4 \\theta + \\cdots) + b (1 + \\cos^2 \\theta + \\cos^4 \\theta + \\cdots)
]

Each series is geometric with first term 1 and common ratio (\\cos^2 \\theta), so:

[
S = (a + b) \\cdot \\frac{1}{1 - \\cos^2 \\theta}
]

Substitute (\\cos \\theta = \\frac{b}{a}):

[
S = (a + b) \\cdot \\frac{1}{1 - \\left(\\frac{b}{a}\\right)^2} = (a + b) \\cdot \\frac{a^2}{a^2 - b^2} = \\frac{a^2 (a + b)}{(a - b)(a + b)} = \\frac{a^2}{a - b}
]
...
The limit of the sum of the perpendicular segments is **(E) (\\frac{a^2}{a - b})**.
