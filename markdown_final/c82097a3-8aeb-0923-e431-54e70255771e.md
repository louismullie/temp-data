...
10 × 84 Given that OA and OB are two different lines, take any point on OA and draw a perpendicular to OB, then from this foot of the perpendicular draw a perpendicular to $O A$, and from the second foot of the perpendicular draw a perpendicular to $O B$, and so on to infinity. If the lengths of the first two perpendicular segments are a, b (a ≠ b), then the sum of these perpendicular segments, as the number of perpendiculars increases indefinitely, approaches a limit. This limit value is
(A) [b/(a − b)].
(B) [a/(a − b)].
(C) [ab/(a − b)].
(D) [b²/(a − b)].
