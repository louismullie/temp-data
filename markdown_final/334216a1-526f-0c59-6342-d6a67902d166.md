...
> Wait, let me verify whether ICD-10-CM distinguishes among XP complementation groups. I need to check if there are separate codes for XPA, XPC, XPG, etc. Hold on, I should verify this carefully because some coding systems are more granular than others. The ICD-10-CM index lists only Q82.1 for xeroderma pigmentosum without subtyping by group, so XPA, XPC, XPG, and XPV are all coded the same way under Q82.1, even though clinically and genetically they differ in severity and neurologic risk [^notfound].
