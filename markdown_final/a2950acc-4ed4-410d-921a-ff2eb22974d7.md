...
The geometric mean length of stay (GMLOS) is the **antilog of the mean of log-transformed hospital stays**, which reduces the influence of extreme outliers in right-skewed LOS data. It is the **preferred metric for LOS** because it better represents typical stays and supports ratio-based comparisons (e.g. 20% reduction) [^112znDLj]. GMLOS is widely used in clinical research, quality benchmarking, and hospital operations to enable fair comparisons across patient groups, interventions, or institutions [^111cfHzi] [^111EvPWt].
