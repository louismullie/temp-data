...
> Wait, let me verify why we prefer this metric for LOS. Hospital length of stay is commonly right-skewed with a long tail of prolonged stays. The geometric mean is less influenced by extreme values than the arithmetic mean, so it better reflects the central tendency in skewed distributions and is often used alongside log-transformation in regression models to meet distributional assumptions [^111SJMUG].
