...
> But wait, what if I misremembered and the equivalence pH were neutral? I should double-check that. No, for weak base–strong acid titrations the equivalence pH is indeed acidic because the conjugate acid hydrolyzes to produce H+. Only strong acid–strong base titrations yield pH 7 at equivalence, so options B, D, and F asserting pH = 7 at equivalence are incorrect for this scenario.

> Next, I should review the clearly wrong trajectories. Options C, E, and G describe pH increasing throughout or ending above 7, which contradicts adding acid to a base. Options A and I introduce pOH equals pKb or pKa at equivalence, which is a mix-up; those relationships pertain to different analytical contexts, not the equivalence point of this titration. Option F gets the shape right but the equivalence pH wrong, so it is close but ultimately incorrect.

> Final answer: H. Initially, the pH slowly decreases, then there is a rapid decrease to give a pH below 7 at the equivalence point.

---

The correct answer is **H**. Initially, the pH decreases slowly as the weak base buffers the added strong acid, then drops rapidly near the equivalence point as the buffer is exhausted, and the equivalence point pH is **below 7** because the solution contains the conjugate acid of the weak base.
