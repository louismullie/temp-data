...
3. The maximum value of the function $y = \sin 2x-2(\sin x+\cos x)$ is $\qquad$.
...
The maximum value of (y = \\sin 2x - 2(\\sin x + \\cos x)) is (1 + 2\\sqrt{2}). This is found by substituting (t = \\sin x + \\cos x), which transforms the function into (y = t^2–2t - 1) with (t \\in [-\\sqrt{2}, \\sqrt{2}]). The maximum occurs at (t = -\\sqrt{2}), yielding (y = 1 + 2\\sqrt{2}).
...
[
y = 2\\left(\\frac{t^2–1}{2} - t \\right) = t^2–2t - 1
]
...
Since (\\sin\\left(x + \\frac{\\pi}{4}\\right) \\in [-1, 1]), the range of (t) is:
...
The function (y = t^2–2t - 1) is a quadratic with a positive leading coefficient, so it opens upward and has a minimum at the vertex. The maximum on a closed interval occurs at one of the endpoints. Evaluate at (t = -\\sqrt{2}) and (t = \\sqrt{2}):

- **At (t = -\\sqrt{2})**:

[
y = (-\\sqrt{2})^2–2(-\\sqrt{2}) - 1 = 2 + 2\\sqrt{2} - 1 = 1 + 2\\sqrt{2}
]
...
Since (1 + 2\\sqrt{2} > 1–2\\sqrt{2}), the maximum value is (1 + 2\\sqrt{2}).

---

## Step 5: Verify the result

The maximum occurs when (t = -\\sqrt{2}), which corresponds to (\\sin x + \\cos x = -\\sqrt{2}). This is achievable when (\\sin\\left(x + \\frac{\\pi}{4}\\right) = -1), confirming the maximum is attainable.

---

The maximum value of (y = \\sin 2x - 2(\\sin x + \\cos x)) is (\\boxed{1 + 2\\sqrt{2}}).
