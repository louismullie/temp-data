...
> I should confirm practical prioritization. Given availability, tolerability, and supporting labeling, doxycycline is typically the first-line alternative when penicillin cannot be used, with minocycline as a reasonable alternative if doxycycline is not tolerated or unavailable; tetracycline remains an option but is less commonly used today due to dosing frequency and tolerability considerations, so I will rank doxycycline first, minocycline second, and tetracycline third in my recommendation [^notfound] [^115tRT1w].
