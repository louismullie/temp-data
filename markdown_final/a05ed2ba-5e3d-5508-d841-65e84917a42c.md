...
The sum of all zeros of (f(x)) is **-4**. This is found by first determining (f(1) = [1/4]) and (f(2) = 1) from the functional equation, which gives the numerator as ([1/4]x² + x − 1). Setting the numerator to zero yields the quadratic (x^2 + 4x - 4 = 0), whose roots sum to **-4** by Vieta's formulas.
