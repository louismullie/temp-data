...
$18 \cdot 66$ The base of a triangle is $b$, and its height is $h$. A rectangle is inscribed in the triangle, with its base on the base of the triangle, and its height is $x$. Then the area of the rectangle is
(A) $\frac{b x}{h}(h-x)$.
(B) $\frac{h x}{b}(b-x)$.
(C) $\frac{b x}{h}(h-2 x)$.
(D) $x(b-x)$.
...
The area of the rectangle is **(A) (\\frac{b x}{h}(h-x))**.
...
[
A = w \\cdot x = \\frac{b}{h}(h - x) \\cdot x = \\frac{b x}{h}(h - x)
]
...
- **(A) (\\frac{b x}{h}(h-x))**: Matches the derived expression.
- **(B) (\\frac{h x}{b}(b-x))**: Incorrect dimensions and form.
- **(C) (\\frac{b x}{h}(h-2 x))**: Incorrect subtraction term.
- **(D) (x(b-x))**: Missing the height ratio (\\frac{b}{h}).
- **(E) (x(h-x))**: Missing the base ratio (\\frac{b}{h}).

---

## Conclusion

The correct answer is **(A) (\\frac{b x}{h}(h-x))**.
