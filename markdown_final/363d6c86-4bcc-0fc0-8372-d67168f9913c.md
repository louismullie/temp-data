...
Example 5 Let $P$ be a point inside $\triangle ABC$, and let $D$, $E$, $F$ be the feet of the perpendiculars from $P$ to $BC$, $CA$, $AB$ respectively: Find all points $P$ that minimize $\frac{BC}{PD} + \frac{CA}{PE} + \frac{AB}{PF}$.
(22nd IMO)
...
The point (P) that minimizes (\\frac{BC}{PD} + \\frac{CA}{PE} + \\frac{AB}{PF}) is the **incenter** of triangle (ABC). This is because the incenter is equidistant from all sides, so (PD = PE = PF = r) (the inradius), and the expression becomes (\\frac{BC + CA + AB}{r}), which is minimal since (r) is the largest possible inradius for any interior point. By the Cauchy-Schwarz inequality, any deviation from the incenter increases the sum, confirming the incenter as the unique minimizer.
...
[
\\left(\\frac{BC}{PD} + \\frac{CA}{PE} + \\frac{AB}{PF} \\right) \\left(BC \\cdot PD + CA \\cdot PE + AB \\cdot PF \\right) \\geq (BC + CA + AB)^2
]

The second factor, (BC \\cdot PD + CA \\cdot PE + AB \\cdot PF), equals twice the area of triangle (ABC) (denoted (2\\Delta)), because each term is twice the area of the sub-triangle formed by (P) and two vertices. Thus:

[
\\left(\\frac{BC}{PD} + \\frac{CA}{PE} + \\frac{AB}{PF} \\right) \\cdot 2\\Delta \\geq (BC + CA + AB)^2
]

Rearranging gives:

[
\\frac{BC}{PD} + \\frac{CA}{PE} + \\frac{AB}{PF} \\geq \\frac{(BC + CA + AB)^2}{2\\Delta}
]
...
[
\\frac{BC}{PD}: \\frac{CA}{PE}: \\frac{AB}{PF} = BC \\cdot PD: CA \\cdot PE: AB \\cdot PF
]
...
The point (P) that minimizes (\\frac{BC}{PD} + \\frac{CA}{PE} + \\frac{AB}{PF}) is the **incenter** of triangle (ABC). This result follows from the Cauchy-Schwarz inequality and the geometric property that the incenter is equidistant from all sides of the triangle.
