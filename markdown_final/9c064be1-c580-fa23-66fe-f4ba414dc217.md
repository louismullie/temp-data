...
13.3.6 ** Given $P(3,4)$ is a point inside the circle $x^{2}+y^{2}=64$, and two moving points $A$ and $B$ on the circumference of the circle satisfy $\angle A P B-\frac{\pi}{2}$. A rectangle $A P B Q$ is constructed with $A P$ and $B P$ as adjacent sides. Find the equation of the trajectory of point $Q$.
...
The trajectory of point (Q) is a circle centered at the origin with radius (\\sqrt{103}). This is derived by noting that (\\angle APB = \\frac{\\pi}{2}) implies (AB) is a diameter of the circumcircle of (\\triangle APB), and that the rectangle condition gives (Q = A + B - P). Using the circle equation and the right-angle condition, we eliminate (A) and (B) to find (x_Q^2 + y_Q^2 = 103).

---

## Geometric setup and conditions

- **Circle**: (x^2 + y^2 = 64), center (O(0, 0)), radius 8.
- **Point P**: ((3, 4)), inside the circle.
- **Points A and B**: on the circumference, with (\\angle APB = \\frac{\\pi}{2}).
...
- **Right angle condition**: (\\angle APB = \\frac{\\pi}{2}) implies (AB) is a diameter of the circumcircle of (\\triangle APB).
- **Rectangle property**: Diagonals bisect each other, so the midpoint of (AB) coincides with the midpoint of (PQ).
- **Vector relation**: (Q = A + B - P), since (\\vec{PQ} = \\vec{AB}).

---

## Algebraic derivation

Let (A = (x_1, y_1)) and (B = (x_2, y_2)). The right-angle condition gives:

[
(x_1–3)(x_2–3) + (y_1–4)(y_2–4) = 0
]

Expanding and rearranging:

[
x_1 x_2 + y_1 y_2–3(x_1 + x_2) - 4(y_1 + y_2) + 25 = 0
]

From the rectangle property, (Q = (x_Q, y_Q) = (x_1 + x_2–3, y_1 + y_2–4)), so:

[
x_1 + x_2 = x_Q + 3, \\quad y_1 + y_2 = y_Q + 4
]

Substitute into the expanded right-angle equation:

[
x_1 x_2 + y_1 y_2–3(x_Q + 3) - 4(y_Q + 4) + 25 = 0
]

[
x_1 x_2 + y_1 y_2 = 3x_Q + 4y_Q + 25–9–16 = 3x_Q + 4y_Q
]

Next, use the circle equations (x_1^2 + y_1^2 = 64) and (x_2^2 + y_2^2 = 64). Add them:

[
x_1^2 + x_2^2 + y_1^2 + y_2^2 = 128
]

Rewrite using sums and products:

[
(x_1 + x_2)^2–2x_1 x_2 + (y_1 + y_2)^2–2y_1 y_2 = 128
]

Substitute (x_1 + x_2 = x_Q + 3), (y_1 + y_2 = y_Q + 4), and (x_1 x_2 + y_1 y_2 = 3x_Q + 4y_Q):

[
(x_Q + 3)^2–2(3x_Q + 4y_Q) + (y_Q + 4)^2 = 128
]

Expand and simplify:

[
x_Q^2 + 6x_Q + 9 + y_Q^2 + 8y_Q + 16–6x_Q - 8y_Q = 128
]
...
This is a circle centered at the origin with radius (\\sqrt{103}).
...
Fig. 2 (Panel A) shows waist circumference trajectories for more than the 22 years to differ in cases and control subjects (P <.0001); yearly differences in waist circumference are tabulated in Supplementary Table 4. Waist circumference in those with dementia was higher from year −22 (1.64 cm, P =.04) to year −16 (1.46, P =.04) and lower starting 8 years before dementia. Fig. 2 (Panel B) shows a similar pattern of results for waist-to-hip ratio (Supplementary Table 4 provides yearly differences), with the trajectories in those with dementia being different from the control subjects (P =.0009) and rapid decline close to dementia diagnosis.
