...
[
3^{k+1} = 3 \\cdot 3^k > 3(2k^2 + 1) = 6k^2 + 3
]
...
[
3^n = 3^{-m} = \\frac{1}{3^m} \\quad \\text{and} \\quad 2n^2 + 1 = 2m^2 + 1
]
...
In STE,and. For large b‐values, both intra‐ and extra‐axonal signals decay exponentially fast,,and both of them are negligible. Therefore, the STE does not provide a considerable signal for large b‐values in a two‐compartment model.
...
where r is the doubling rate in 1/hours and and Δ r is the difference in doubling rate between population 2 and population 1. At timethe proportion of cells in population 2 is given by
