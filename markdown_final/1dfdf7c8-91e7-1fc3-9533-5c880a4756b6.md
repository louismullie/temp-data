...
Call a 7-digit telephone number d₁d₂d₃-d₄d₅d₆d₇ memorable if the prefix [sequence](https://artofproblemsolving.com/wiki/index.php/Sequence) d₁d₂d₃ is exactly the same as either of the sequences d₄d₅d₆ or d₅d₆d₇ (possibly both). Assuming that each d_i can be any of the ten decimal digits 0,1,2, …, 9, the number of different memorable telephone numbers is
...
> Let me first confirm the total space. A 7-digit number has 10 choices for each digit, so the total number of possible 7-digit sequences is 10^7, which is 10,000,000. Wait, let me verify that I'm not conflating this with the memorable subset; yes, that's the total universe we are drawing from, and we will later subtract the memorable ones to get the desired count [^notfound].
...
> Next, I will compute |A|. If d1d2d3 = d4d5d6, then digits 1–3 determine digits 4–6, and digit 7 is free. So there are 10^3 choices for the prefix, 1 choice for digits 4–6 (since they must match the prefix), and 10 choices for digit 7, giving 10^3 × 1 × 10 = 10,000 numbers in A. Hold on, I should double-check that I'm not overconstraining digit 7; yes, digit 7 is independent here, so 10,000 is correct for |A| [^notfound].

> Similarly, I will compute |B|. If d1d2d3 = d5d6d7, then digits 1–3 determine digits 5–7, and digit 4 is free. So there are 10^3 choices for the prefix, 10 choices for digit 4, and 1 choice for digits 5–7 (since they must match the prefix), giving 10^3 × 10 × 1 = 10,000 numbers in B. Wait, let me verify the independence again; digit 4 is indeed unconstrained in this case, so 10,000 is correct for |B| [^notfound].
...
Each digit (d_i) can be any of the 10 decimal digits (0–9), so the total number of 7-digit numbers is:
...
A number is memorable if the prefix (d_1d_2d_3) matches either (d_4d_5d_6) or (d_5d_6d_7). We count these using inclusion–exclusion.

---

### Case 1: prefix matches (d_4d_5d_6)
...
### Case 2: prefix matches (d_5d_6d_7)
...
### Overlap: prefix matches both (d_4d_5d_6) and (d_5d_6d_7)
