...
> I need to check the divergence of the vector field. The divergence of a = x i + (y + z) j + (z − y) k is ∂x/∂x + ∂(y + z)/∂y + ∂(z − y)/∂z = 1 + 1 + 1 = 3, which is constant everywhere, so the volume integral will be straightforward.
...
> Wait, let me verify the volume calculation. The volume of a full sphere is 4/3 π r³, so the volume of a hemisphere is half of that, which is 2/3 π r³. Multiplying by 3 gives exactly 2π r³, so 54π is correct for the total flux through the closed surface.

> Now I will examine the flux through the flat disk at z = 0. On the disk, z = 0, so the vector field becomes a = x i + y j − y k, and the outward normal for the closed surface points downward, i.e., n = −k. Therefore, a · n = (x i + y j − y k) · (−k) = y, and the flux through the disk is ∬ over disk y dA. By symmetry, the integral of y over a disk centered at the origin is zero, so Φ_disk = 0. Hold on, I should verify that symmetry argument: yes, for every y there is a corresponding −y, so the integral vanishes, which is correct.

> I should double-check the orientation. The problem states the normal is external to the closed surface, so for the disk that means pointing downward, which is why n = −k is appropriate here. If I mistakenly used +k, I would get the wrong sign for the disk contribution, but in this case the dot product would flip and the integral would still be zero by symmetry, so the final answer would be unchanged, though the reasoning would be inconsistent with the stated normal direction.

> Finally, I will compute the hemisphere flux by subtraction. Since Φ_total = Φ_hemisphere + Φ_disk and Φ_disk = 0, we have Φ_hemisphere = Φ_total = 54π. Let me reconsider whether any steps could have introduced an error: the divergence is correct, the volume is correct, the disk integral is correct by symmetry, and the subtraction is straightforward, so the result should be reliable.
...
∬ over S a · dS = ∭ over V (∇ · a) dV

First, compute the **divergence** of (a):
...
∬ over S_closed a · dS = ∭ over V 3 dV = 3 × 18π = 54π
...
The flux of the vector field (a) through the upper hemisphere is **(54π)**.
...
### Terahertz rectification in ring-shaped quantum barriers [^114kbzNi]. Nature Communications (2018). Medium credibility.
...
### High spin axion insulator [^11188pS7]. Nature Communications (2024). High credibility.
...
### Physical realization of topological roman surface by spin-induced ferroelectric polarization in cubic lattice [^1128EJd6]. Nature Communications (2022). High credibility.
...
### Multi-scale turbulence simulation suggesting improvement of electron heated plasma confinement [^116xeC84]. Nature Communications (2022). High credibility.
...
### Revised estimates of ocean-atmosphere COflux are consistent with ocean carbon inventory [^1159HQvG]. Nature Communications (2020). High credibility.
...
### Confirmation of the topology of the wendelstein 7-X magnetic field to better than 1: 100, 000 [^113ouUBu]. Nature Communications (2016). Medium credibility.
...
### Topology, landscapes, and biomolecular energy transport [^117RadFW]. Nature Communications (2019). High credibility.
...
### Learning non-stationary langevin dynamics from stochastic observations of latent trajectories [^114Vcvec]. Nature Communications (2021). High credibility.
...
### Functional decomposition of metabolism allows a system-level quantification of fluxes and protein allocation towards specific metabolic functions [^116Aazk6]. Nature Communications (2023). High credibility.
...
### A chip-scale atomic beam clock [^116LBMxP]. Nature Communications (2023). High credibility.
...
### Flow perturbation to accelerate boltzmann sampling [^111hmS2B]. Nature Communications (2025). High credibility.
...
### Flow perturbation to accelerate boltzmann sampling [^113AfRFh]. Nature Communications (2025). High credibility.
...
### Reverberant 3D optical coherence elastography maps the elasticity of individual corneal layers [^112gErWi]. Nature Communications (2019). High credibility.
...
### X-ray rheography uncovers planar granular flows despite non-planar walls [^115gAHWa]. Nature Communications (2018). Medium credibility.
...
### Network dynamics: jamming is limited in scale-free systems [^1156i5Gu]. Nature (2004). Excellent credibility.
...
### Controllable quantum point junction on the surface of an antiferromagnetic topological insulator [^114vRxqY]. Nature Communications (2021). High credibility.
...
### Half-quantum mirror hall effect [^111UvUzt]. Nature Communications (2024). High credibility.
...
### Edge States and skyrmion dynamics in nanostripes of frustrated magnets [^115Eu6pH]. Nature Communications (2017). Medium credibility.
...
### An analytic approximation of the feasible space of metabolic networks [^114ufqB6]. Nature Communications (2017). Medium credibility.
...
### Dynamic simulation of articulated soft robots [^113cPhKc]. Nature Communications (2020). High credibility.
...
### Mesoscale atmosphere ocean coupling enhances the transfer of wind energy into the ocean [^114dC1CU]. Nature Communications (2016). Medium credibility.
...
### Deep-subwavelength control of acoustic waves in an ultra-compact metasurface lens [^114z8xJv]. Nature Communications (2018). Medium credibility.
...
### In silico voltage-sensitive dye imaging reveals the emergent dynamics of cortical populations [^115aSw81]. Nature Communications (2021). High credibility.
...
### In situ single-shot diffractive fluence mapping for X-ray free-electron laser pulses [^111CieES]. Nature Communications (2018). Medium credibility.
...
### In situ fully vectorial tomography and pupil function retrieval of tightly focused fields [^111kqnZj]. Nature Communications (2025). High credibility.
...
### Twist, splay, and uniform domains in ferroelectric nematic liquid crystals [^116wXh5a]. Nature Communications (2025). High credibility.
...
### Topological superconducting vortex from trivial electronic bands [^116qUMFn]. Nature Communications (2023). High credibility.
...
### MRI-based axis-referenced morphometric model corresponding to lamellar organization for assessing hippocampal atrophy in dementia [^115nNTYp]. Human Brain Mapping (2024). Medium credibility.
...
### Switching of band inversion and topological surface States by charge density wave [^112H7WnB]. Nature Communications (2020). High credibility.
...
### Dichotomy of heavy and light pairs of holes in the t-J model [^116FjKZ3]. Nature Communications (2023). High credibility.
...
### Switching of band inversion and topological surface States by charge density wave [^114snFqp]. Nature Communications (2020). High credibility.
...
### Machine learning assisted vector atomic magnetometry [^117Duth3]. Nature Communications (2023). High credibility.
...
### Fermi surface tomography [^116TEeDV]. Nature Communications (2022). High credibility.
...
### Manipulation and control of droplets on surfaces in a homogeneous electric field [^114Kr76z]. Nature Communications (2022). High credibility.
...
### Antibody binding reports spatial heterogeneities in cell membrane organization [^116iRhnC]. Nature Communications (2023). High credibility.
...
### Machine learning enables completely automatic tuning of a quantum device faster than human experts [^1173DLJH]. Nature Communications (2020). High credibility.
...
### An integrated imaging sensor for aberration-corrected 3D photography [^112M9W4s]. Nature (2022). Excellent credibility.
...
### Energetic optimization of ion conduction rate by the K + selectivity filter [^111QnNZV]. Nature (2001). Excellent credibility.
...
### Realizing repeated quantum error correction in a distance-three surface code [^117USzuX]. Nature (2022). Excellent credibility.
...
### Reduced order modeling with shallow recurrent decoder networks [^112CoZNp]. Nature Communications (2025). High credibility.
...
### Quantifying the contribution of dominance deviation effects to complex trait variation in biobank-scale data [^1137gi1g]. American Journal of Human Genetics (2021). Medium credibility.
...
### Causal mediation analysis for time-varying heritable risk factors with mendelian randomization [^111bqZLg]. Nature Communications (2025). High credibility.
...
### Helicity dependent photocurrent in electrically gated (BiSb) tethin films [^1142wFv1]. Nature Communications (2017). Medium credibility.
...
### Seasonal and diurnal variations of kelvin-helmholtz instability at terrestrial magnetopause [^114hpwg6]. Nature Communications (2023). High credibility.
...
### Morphology-independent general-purpose optical surface tractor beam [^113dqZLo]. Nature Communications (2024). High credibility.
...
### Lorentz-violating type-II dirac fermions in transition metal dichalcogenide PtTe [^116uNahu]. Nature Communications (2017). Medium credibility.
...
### Quantum error correction below the surface code threshold [^113zxFFb]. Nature (2025). Excellent credibility.
...
### Transition path times of coupled folding and binding reveal the formation of an encounter complex [^112cV5HC]. Nature Communications (2018). Medium credibility.
...
### A 3D hybrid-shot spiral sequence for hyperpolarizedimaging [^111veZFb]. Magnetic Resonance in Medicine (2021). Medium credibility.
...
### Bloch surface waves confined in one dimension with a single polymeric nanofibre [^11397ivC]. Nature Communications (2017). Medium credibility.
...
### Evidence for dynamic kagome ice [^113m44D3]. Nature Communications (2018). Medium credibility.
...
### Rotating curved spacetime signatures from a giant quantum vortex [^115KKy2P]. Nature (2024). Excellent credibility.
...
### Dislocation majorana bound States in iron-based superconductors [^115ErPti]. Nature Communications (2024). High credibility.
...
### Filgrastim (Neupogen) [^115tWNXu]. FDA (2025). Medium credibility.
...
### Reactivity of shape-controlled crystals and metadynamics simulations locate the weak spots of alumina in water [^116Q2RnF]. Nature Communications (2019). High credibility.
...
### 1.1% sodium fluoride (Sf) [^112bqDTs]. FDA (2016). Low credibility.
