...
$36 \cdot 44 \triangle ABC$ has side lengths of $6, 8, 10$, with the center at $P$, and a radius of 1. A circle rolls inside $\triangle ABC$, always remaining tangent to at least one side. When $P$ first returns to its initial position, the distance traveled by $P$ is
(A) 10.
(B) 12.
(C) 14.
(D) 15.
...
- **Side lengths**: 6, 8, 10.
- **Type**: Right-angled triangle (since (6^2 + 8^2 = 10^2)).
- **Area**: (\\frac{1}{2} \\times 6 \\times 8 = 24).
- **Inradius**: (r = \\frac{A}{s} = \\frac{24}{12} = 2), where (s) is the semi-perimeter.
...
The inradius of ABC is 2, so the inradius of the inner triangle is (2–1 = 1). Because the triangles are similar, the ratio of corresponding sides is the ratio of inradii, which is (\\frac{1}{2}). Thus, the side lengths of the inner triangle are:

- **Side 1**: (6 \\times \\frac{1}{2} = 3)
- **Side 2**: (8 \\times \\frac{1}{2} = 4)
- **Side 3**: (10 \\times \\frac{1}{2} = 5)
