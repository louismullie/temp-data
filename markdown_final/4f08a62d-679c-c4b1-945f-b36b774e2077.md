...
> But wait, what if the starting value is too small? I should confirm the domain. The expression under the square root requires (2xₙ₋₁ + 3 ≥ 0), so (xₙ₋₁ ≥ −[3/2]). If (x₁ < −[3/2]), the first term (x₂) is undefined, so the sequence does not exist beyond the first term. Let me reconsider: if (x₁ = −[3/2]), then (x₂ = 0), and from there the sequence increases toward 3, so even in that boundary case it converges to 3. Thus, the only problematic scenario is (x₁ < −[3/2]), where the sequence is undefined after the first step [^notfound].
...
> Final answer: The limit is 3 for any initial value (x₁ ≥ −[3/2]). If (x₁ < −[3/2]), the sequence is undefined after the first term, so no limit exists in that case [^notfound].
