...
The equation ([3/19]^x + [5/19]^x + [11/19]^x = 2√(x − 1)) has **exactly one solution**. The left side is a strictly decreasing exponential sum, while the right side is a strictly increasing square root function; they intersect exactly once in the domain (x ≥ 1). Thus, the correct answer is **(B) 1**.
